/datum/surgery_operation/limb/bionecrosis
	...
	desc = "Ввести реагенты, стимулирующие рост Ромерол-опухоли в мозге пациента."
...
/datum/surgery_operation/limb/bionecrosis/on_preop(obj/item/bodypart/limb, mob/living/surgeon, obj/item/tool, list/operation_args)
	display_results(
		surgeon,
		limb.owner,
		span_notice("Вы начинаете выращивать Ромерол-опухоль в мозге [limb.owner]..."),
		span_notice("[surgeon] начинает копаться в мозге [limb.owner]..."),
		span_notice("[surgeon] начинает операцию на мозге [limb.owner]."),
	)
...
/datum/surgery_operation/limb/bionecrosis/on_success(obj/item/bodypart/limb, mob/living/surgeon, obj/item/tool, list/operation_args)
	display_results(
		surgeon,
		limb.owner,
		span_notice("Вам удаётся вырастить Ромерол-опухоль в мозге [limb.owner]."),
		span_notice("[surgeon] успешно выращивает Ромерол-опухоль в мозге [limb.owner]!"),
		span_notice("[surgeon] завершает операцию на мозге [limb.owner]."),
	)
