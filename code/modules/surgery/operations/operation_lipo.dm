...
/datum/surgery_operation/limb/lipoplasty/all_required_strings()
	. = list()
	. += "оперировать грудную клетку (цель — грудь)"
	. += ..()
	. += "у пациента должен быть лишний жир для удаления"
...
/datum/surgery_operation/limb/lipoplasty/on_preop(obj/item/bodypart/limb, mob/living/surgeon, obj/item/tool, list/operation_args)
	display_results(
		surgeon,
		limb.owner,
		span_notice("Вы начинаете срезать лишний жир [limb.owner.declent_ru(GENITIVE)]..."),
		span_notice("[surgeon] начинает срезать лишний жир [limb.owner.declent_ru(GENITIVE)]."),
		span_notice("[surgeon] начинает резать [limb.ru_plaintext_zone[ACCUSATIVE]] [limb.owner.declent_ru(GENITIVE)] с помощью [tool]."),
	)
...
/datum/surgery_operation/limb/lipoplasty/on_success(obj/item/bodypart/limb, mob/living/surgeon, obj/item/tool, list/operation_args)
	display_results(
		...
		span_notice("[surgeon] заканчивает срезать лишний жир из [limb.ru_plaintext_zone[GENITIVE]] [limb.owner.declent_ru(GENITIVE)]."),
	)
