...
/datum/surgery_operation/limb/autopsy/on_preop(obj/item/bodypart/limb, mob/living/surgeon, obj/item/autopsy_scanner/tool, list/operation_args)
	display_results(
		surgeon,
		limb.owner,
		span_notice("Вы начинаете проводить вскрытие [limb.owner]..."),
		span_notice("[surgeon] использует [tool] для проведения вскрытия [limb.owner]."),
		span_notice("[surgeon] использует [tool] на груди [limb.owner]."),
	)
