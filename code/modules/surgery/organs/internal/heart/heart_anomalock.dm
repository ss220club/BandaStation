...
/obj/item/organ/heart/cybernetic/anomalock
	...
	desc = "Обогнавшее своё время кибернетическое сердце, первоначально разработанное для 'Отрядов Зачистки' Нанотрейзен, но позже рассекреченное для обычных изучений. Гальваническая технология позволяет удерживать тело в вертикальном положении в тяжёлых условиях, а также перенаправлять аномальную поточную энергию для защиты от электромагнитных импульсов. Для работы требует Флюкс ядро."
...
/obj/item/organ/heart/cybernetic/anomalock/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(!istype(tool, required_anomaly))
		return NONE
	if(core)
		balloon_alert(user, "ядро уже внутри!")
