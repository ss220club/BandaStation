...
/datum/customer_data/american
	...
	friendly_pull_line = "Куда вы меня везёте? Надеюсь, не в медотдел, у меня нет страховки."
...
/datum/customer_data/moth
	...
	leave_mad_lines = list("Я говорю всем моим друзьям-молькам, чтобы они никогда сюда не приходили!", "Ноль звёзд! Даже молберт на вкус был лучше!","Полное закрытие было бы слишком простой судьбой для этого места.")
...
/datum/customer_data/malfunction
	...
	leave_mad_lines = list("Время выполнения robot_customer_controller.dm, строка 28: неопределен тип пути /datum/ai_behavior/leave_venue.", "ЕСЛИ БЫ У ВАС БЫЛ 5 ИНТЕНТ Я БЫ ВАС УДАРИЛ!", "Я расскажу об этом богам.")
