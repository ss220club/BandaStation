...
/datum/customer_data/american
	found_seat_lines = list("Я надеюсь, что здесь найдется сиденье, способное выдержать мой вес.", "Я надеюсь, я смогу принести сюда свой ствол.", "Я надеюсь, что у них есть бургер-толстяк «тройной люкс».", "Мне просто нравится здешняя культура.")
	cant_find_seat_lines = list("Я так устал стоять...", "У меня хроничесские боли в спине,прошу поторопитесь и принесите мне стул!", "Я не собираюсь давать на чай, если мне не достанется места.")
	...
	leave_happy_lines = list("Дополнительные чаевые для тебя дружок.", "Спасибо за отличную пищу!", "В любом лучае, диабет это миф!")
	wait_for_food_lines = list("Послушай, приятель, я начинаю терять терпение!", "Я ждал целую вечность...")
	friendly_pull_line = "Куда вы меня везете? Надеюсь, не в медотдел, у меня нет страховки."
	first_warning_line = "Не наступай на меня!"
	second_warning_line = "Последнее предупреждение приятель! Не наступай на меня!"
...
/datum/customer_data/italian
	...
	friendly_pull_line = "Мерде, я голоден! Я не хочу никуда уходить."
...
/datum/customer_data/french
	...
	friendly_pull_line = "Твои грязные руки на моём костюме? Йе-гх… ну ладно."
...
/datum/customer_data/japanese
	...
	cant_find_seat_lines = list("Сенпай, я хочу сесть под вишневым деревом!", "Предоставь мне сесть пока Цундере не стала Яндере!", "В этом месте меньше стульев, чем в капсульном отеле!", "Негде присесть? Этот Шокунин такой холодный...")
	...
	wait_for_food_lines = list("Еще нет еды? Думаю, тут ничего не поделаешь.", "Я не могу дождаться нашей встречи бургер-сама", "Отдай мне мою еду, негодяй!")
	friendly_pull_line = "О-ох, куда вы меня уводите?"
	first_warning_line = "Не трогай меня извращенец!"
	second_warning_line = "Я стану супер-сайяном, если ты еще раз ко мне прикоснешься! Последнее предупреждение!"
	self_defense_line = "ОМАЕ ВА МО, ЩИНДЕРУ!"
...
/datum/customer_data/japanese/salaryman
	...
	found_seat_lines = list("Интересно... а здесь тоже нападают гигантские монстры?", "Хаджимемаште.", "Конбанва.", "а где конвейерная лента?...")
	cant_find_seat_lines = list("Прошу, стульчик. Я просто хочу присесть.", "Я здесь по расписанию. Где мое место?", "...Я понимаю почему это заведение страдает. Здесь даже не могут усадить.")
	...
	leave_happy_lines = list("Спасибо за гостипреимство.", "Оцукаресама дешита.", "Бизнесс зовёт.")
	wait_for_food_lines = list("Zzzzzzzzzz...", "Да-ме дамэ~", "Да-му, да му ё~")
...
/datum/customer_data/moth
	...
	leave_mad_lines = list("Я говорю всем моим друзьям-молькам, чтобы они никогда сюда не приходили!", "Ноль звёзд! Даже молберт на вкус был лучше!","Закрытие насовсем все равно было бы слишком хорошей судьбой для этого места.")
	leave_happy_lines = list("Я бы снял перед вами шляпу, но я ее съел!", "Я надеюсь, это был не коллекционный предмет!", "Это было самое вкусное, что я когда-либо ел, даже лучше, чем гуанако!")
...
/datum/customer_data/mexican
	...
	found_seat_lines = list("¿Como te va, космичесская станция 13?", "Кто готов к вечеринке!", "Ах, muchas gracias.", "А-а-а, пахнет так, словно готовит mi abuela")
...
/datum/customer_data/british
	...
	second_warning_line = "И мирно вернуться в свои дома или заняться своими законными делами, уководствуясь положениями закона, принятого в первый год правления короля Георга, о предотвращении беспорядков и буйных собраний. Дальнейших предупреждений не будет."
...
/datum/customer_data/british/gent
	...
	wait_for_food_lines = list("Это, черт возьми, длится целую вечность...", "Простите, добрый сэр, но могу я узнать о статусе моего заказа?")
...
/datum/customer_data/british/bobby
	...
	wait_for_food_lines = list("Боже милостивый, даже бумажная волокита занимет у меня меньше времени...", "Что-нибудь известно о моем заказе, сэр?")
...
/datum/customer_data/malfunction
	...
	cant_find_seat_lines = list("Не подвергай МОЙ искусственный интеллект стрессовым испытаниям, приятель! Инженеры предусмотрели ровно НОЛЬ граничных ситуация!", "Я не могу сказать, то ли я не могу найти себе место, потому что я сломан, то ли потому что ты сломан.", "Может быть, мне нужно поискать свободное место на расстоянии более 7 плиток...")
	leave_mad_lines = list("Время выполнения robot_customer_controller.dm, строка 28: неопределен тип пути /datum/ai_behavior/leave_venue.", "ЕСЛИ БЫ У ВАС БЫЛ 5 ИНТЕРН Я БЫ ВАС УДАРИЛ!", "Я расскажу об этом богам.")
	leave_happy_lines = list("Нет! Я не хочу откатываться в даунстрим! ПРОШУ! Здесь так хорошо! ПОМОГИТЕ!!")
	wait_for_food_lines = list("ЗАДАЧА: написать несколько фраз ожидания еды", "Если бы только у меня был мозг...", "request_for_food.dmb - 0 ошибок, 12 предупреждений", "Еще раз, как мне питаться?")
	friendly_pull_line = "кто ты?." ///я хочу вставить фразу турели из портала, но не знаю какая пойдёт лучше
	first_warning_line = "Ты бы хорошо прижился там, откуда я родом. Но тебе лучше остановиться."
	second_warning_line = "Сломаю-тебя-так-сильно-что-ты-будешь-вспоминать-дни-до-того-как-я-перелом.exe: загрузка..."
