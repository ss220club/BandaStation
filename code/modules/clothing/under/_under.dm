...
/obj/item/clothing/under/proc/repair_sensors(mob/user)
	if(has_sensor != BROKEN_SENSORS)
		if(user)
			balloon_alert(user, "сенсоры [has_sensor == NO_SENSORS ? "не найдены" : "не сломаны"]!")
		return FALSE
...
/obj/item/clothing/under/examine(mob/user)
	...
	if(has_sensor == BROKEN_SENSORS)
		...
		. += span_tooltip("Ты всегда можешь получить новые сенсоны из автолата", "Не надеты медицинские датчики.")
