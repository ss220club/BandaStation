...
/obj/item/clothing/head/helmet/perceptomatrix/proc/pre_cast_core_check(mob/caster, datum/action/cooldown/spell/spell)
	...
	if((!core_installed) && spell.school == SCHOOL_PSYCHIC)
		to_chat(caster, span_warning("Вы не можете воздействовать на разум без установленного ядра!"))
...
/datum/action/cooldown/spell/pointed/percept_hallucination/cast(mob/living/carbon/human/cast_on)
	...
	if(cast_on.can_block_magic(antimagic_flags))
		to_chat(cast_on, span_notice("Вы чувствуете, как галлюцинаторная энергия отражается от вас."))
		to_chat(owner, span_warning("[cast_on] отклоняет галлюцинаторную энергию!"))
		return

	to_chat(cast_on, span_warning("Ваш мозг будто горит!"))
