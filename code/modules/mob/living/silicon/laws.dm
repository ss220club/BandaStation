...
/mob/living/silicon/proc/deadchat_lawchange()
	...
	deadchat_broadcast(" <b>законы были изменены.</b> <a href='byond://?src=[REF(src)]&dead=1&printlawtext=[url_encode(lawtext)]'>Посмотреть</a>", span_name("[src]"), follow_target=src, message_type=DEADCHAT_LAWCHANGE)
