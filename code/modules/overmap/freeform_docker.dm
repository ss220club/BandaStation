/datum/shuttle_freeform_docker
	var/mob/eye/camera/remote/shuttle_freeform/eyeobj
...
/datum/shuttle_freeform_docker/proc/StartCameraView()
	...
	if(!CreateEye())
		to_chat(current_user, span_warning("\The [src] flashes a bunch of never-ending errors on the display. Something is really wrong."))
		return
...
/datum/shuttle_freeform_docker/proc/give_eye_control()
	GrantActions()
	eyeobj.assign_user(current_user)
...
/datum/shuttle_freeform_docker/proc/PlaceLandingSpot()
	...
	if(my_shuttle.freeform_port?.get_docked())
		...
		my_shuttle.freeform_port.shuttle_template_id = null
...
/mob/eye/camera/remote/shuttle_freeform
	var/list/placement_images = list()

/mob/eye/camera/remote/shuttle_freeform/setLoc(T, force_update = TRUE)
	..()
	var/datum/shuttle_freeform_docker/docker = origin_ref
	if(docker)
		docker.checkLandingSpot()

/mob/eye/camera/remote/shuttle_freeform/update_remote_sight(mob/living/user)
