...
/datum/action/cooldown/spell/list_target/telepathy/cast(mob/living/cast_on)
	...
	owner.cast_tts(owner, message, is_local = FALSE, channel_override = CHANNEL_TTS_RADIO, check_deafness = FALSE) // BANDASTATION ADDITION - TTS
	if(!cast_on.can_block_magic(antimagic_flags, charge_cost = 0)) //hear no evil
		...
		owner.cast_tts(cast_on, message, is_local = FALSE, channel_override = CHANNEL_TTS_RADIO, check_deafness = FALSE) // BANDASTATION ADDITION - TTS
	else
