...
/obj/machinery/satellite/meteor_shield/proc/handle_new_emagged_shield_threshold()
	switch(highest_emagged_threshold_reached)
		...
		if(EMAGGED_METEOR_SHIELD_THRESHOLD_THREE)
			...
			priority_announce("Внимание! Вмешательство в работу метеорных спутников подвергает станцию ​​риску необычных и смертоносных столкновений с метеоритами. Пожалуйста, проверьте свои GPS-устройства на наличие странных сигналов и демонтируйте взломанные метеоритные щиты.", "Предупреждение о странном метеоритном сигнале")
