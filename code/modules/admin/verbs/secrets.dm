...
/proc/do_portal_spawn(turf/loc, mobtype, numtospawn, list/portal_appearance, players, humanoutfit)
	for (var/i in 1 to numtospawn)
		...
		if (length(players))
			...
			if (chosen.client)
				chosen.client.prefs.safe_transfer_prefs_to(spawned_mob, is_antag = TRUE, do_not_apply = list(/datum/preference/body_modifications)) // BANDASTATION MOD - Do not apply body mods on roles
