...
/datum/admins/proc/make_emergency_response_team(datum/ert/ertemplate = null, skip_preference_menu = FALSE) // BANDASTATION EDIT - ERT request panel
	...
	if(ertemplate.spawn_admin)
		if(isobserver(usr))
			...
			usr.client.prefs.safe_transfer_prefs_to(admin_officer, is_antag = TRUE, do_not_apply = list(/datum/preference/body_modifications)) // BANDASTATION MOD - Do not apply body mods on roles
	...
	while(numagents && candidates.len)
		...
		if(ertemplate.mob_type)
			...
			chosen_candidate.client.prefs.safe_transfer_prefs_to(ert_operative, is_antag = TRUE, do_not_apply = list(/datum/preference/body_modifications)) // BANDASTATION MOD - Do not apply body mods on roles
