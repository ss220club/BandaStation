...
/datum/job/proc/get_spawn_mob(client/player_client, atom/spawn_point, late_join = FALSE)  // BANDASTATION EDIT - Pref Job Slots
	...
	spawn_instance.apply_prefs_job(player_client, src, late_join) // BANDASTATION EDIT - Pref Job Slots
	if(!player_client)
		qdel(spawn_instance)
...
/mob/living/carbon/human/apply_prefs_job(client/player_client, datum/job/job, late_join = FALSE)  // BANDASTATION EDIT - Pref Job Slots
	...
	var/randomise_job_slot = player_client.prefs.set_assigned_slot(job.title, late_join)  // BANDASTATION ADD - Pref Job Slots
