...
/obj/item/vorpalscythe/Initialize(mapload)
	. = ..()
	AddElement(/datum/element/nullrod_core, chaplain_spawnable = FALSE, rune_remove_line = "В ПЫЛЬ С ТОБОЮ!! ПРОЧЬ!!") // The implant is the actual item the chappie can select
	AddComponent(
