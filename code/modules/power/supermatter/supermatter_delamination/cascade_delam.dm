...
/datum/sm_delam/cascade/delaminate(obj/machinery/power/supermatter_crystal/sm)
	...
	priority_announce("В вашем секторе произошло событие резонансного сдвига типа С. Сканирование указывает на локальный поток колебаний, влияющий на пространственную и гравитационную субструктуру. \
		Образовалось несколько резонансных точек. Пожалуйста, ожидайте.", "Ассоциация Наблюдения За Звездами Нанотрейзен", ANNOUNCER_SPANOMALIES)
	sleep(2 SECONDS)
...
/datum/sm_delam/cascade/count_down_messages(obj/machinery/power/supermatter_crystal/sm)
	var/list/messages = list()
	messages += "РАССЛОЕНИЕ КРИСТАЛЛА НЕИЗБЕЖНО. Сверхматерия достигла критического нарушения целостности. Превышены пределы гармонических частот. Поле дестабилизации причинности не может быть задействовано."
	messages += "Кристаллическая гиперструктура возвращается к безопасным рабочим параметрам. Гармоническая частота восстановлена в пределах аварийных границ. Запущен антирезонансный фильтр."
...
/datum/sm_delam/cascade/proc/announce_cascade(obj/machinery/power/supermatter_crystal/sm)
	...
	if(!can_select(sm))
		return FALSE
	priority_announce("Внимание: Сканирование аномалий дальнего действия указывает на аномальное количество гармонического потока, исходящего от \
	объекта в пределах [station_name()], может произойти резонансный коллапс.",
	"Ассоциация Наблюдения За Звездами Нанотрейзен", 'sound/announcer/alarm/airraid.ogg')
	return TRUE
