		...
		for(var/dy = -4 to 4)
			...
			if(expected_lit)
				TEST_ASSERT_EQUAL(
					actual_lum,
					expected_power,
					"Expected directional light to affect [T], but dynamic lumcount was [actual_lum]."
				)
			else
				TEST_ASSERT_EQUAL(
					actual_lum,
					0,
					"Expected directional light to not affect [T], but dynamic lumcount was [actual_lum]."
				)
