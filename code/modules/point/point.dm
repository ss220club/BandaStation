...
/atom/movable/proc/point_at(atom/pointed_atom, intentional = FALSE)
	...
	animate(visual, pixel_x = (tile.x - our_tile.x) * ICON_SIZE_X + pointed_atom.pixel_x, pixel_y = (tile.y - our_tile.y) * ICON_SIZE_Y + pointed_atom.pixel_y, time = POINT_TIME / 4.5, easing = CIRCULAR_EASING|EASE_IN) // BANDASTATION EDIT: 'time = 1.7' => 'time = POINT_TIME / 4.5' & 'easing = CIRCULAR_EASING|EASE_IN'
	return TRUE
