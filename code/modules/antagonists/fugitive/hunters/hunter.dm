...
/datum/antagonist/fugitive_hunter/greet()
	switch(backstory)
		...
		if(HUNTER_PACK_RUSSIAN)
			to_chat(owner, span_danger("Ай бля. Я космический русский контрабандист! Мы были в полете, когда наш груз телепортировали с нашего корабля!"))
		...
		if(HUNTER_PACK_BOUNTY)
			...
			to_chat(owner, span_danger("Разведданные сообщали, что наша цель находится на исследовательской станции. Необычное место для преступника, чтобы залечь на дно."))
