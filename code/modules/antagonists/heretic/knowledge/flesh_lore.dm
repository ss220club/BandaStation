/datum/heretic_knowledge_tree_column/flesh
	...
	description = list(
		"Путь Плоти специализируется на призыве управляемых игроком миньонов и демонов для выполнения ваших приказов.",
	...
	cons = list(
		"Важным этапом вашего прогресса является получение дополнительных призываемых монстров.",
		"У вас очень мало полезных способностей, кроме призыва монстров.",
	...
	tips = list(
		"Ваша Хватка Мансуса позволяет вам превращать мертвых гуманоидов в гулей (даже гуманоидов с защитой разума, таких как офицеры и капитан). Он также оставляет след, который вызывает сильное кровотечение, когда его задевает ваш Кровавый Клинок.",
		"Для Еретика Плоти органы и трупы — лучшие друзья! Ты можешь использовать их для ритуалов, исцеления или получения усилений.",
		"Ваше заклинание «Хирургия Плоти» может исцелять ваших призываемых существ. Ваша мантия наделяет вас аурой, которая также исцеляет призываемых существ, находящихся поблизости (но не вас самих).",
		"Ваше заклинание «Хирургия Плоти» также позволяет вам похищать органы у гуманоидов. Полезно, если вам нужна запасная печень.",
		...
		"Сталкеры Плоти — неплохие бойцы, способные маскироваться под мелких существ, таких как бипски и корги. Они также могут использовать заклинание ЭМИ, но это может нанести им вред, если они превратились в робота!",
...
/datum/heretic_knowledge/spell/flesh_surgery
	name = "Хирургия Плоти"
	desc = "Дарует вам заклинание Хиругия Плоти. Это заклинание позволяет извлекать органы из жертв \
