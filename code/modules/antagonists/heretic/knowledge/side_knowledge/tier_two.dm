...
/datum/heretic_knowledge/codex_morbus
	name = "Кодекс Болезни"
	desc = "Позволяет трансмутировать КОдекс Шрама и тело, в Кодекс Болезни. \
	...
	gain_text = "Корешок этого тома в кожаном переплете скрипит с жутким, страдальческим вздохом. \
		...
		Оно вещает об иглах, что пронзают кожу мироздания, однажая его и оставляя гнить. И оно зовёт меня по имен."
	required_atoms = list(
...
/datum/heretic_knowledge/greaves_of_the_prophet
	name = "Поножи Пророка"
	desc = "Позволяет объеденить пару ботинок с двумя листами титана или серебра, чтобы получить пару бронированных понож, обеспечивающих полную защиту от подскальзывания."
	gain_text = " \
...
/datum/heretic_knowledge/ether
	...
	gain_text = "Зрение и мысли затуманиваются, когда я вдыхаю пары этого ихора. \
		...
		Это жалкое создание, вырванное из тумана снов, я преподношу своей. Каковы же мы глупцы."
	required_atoms = list(
		/obj/item/shard = 1,
