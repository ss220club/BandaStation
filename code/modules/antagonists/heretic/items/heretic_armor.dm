...
/obj/item/clothing/suit/hooded/cultrobes/void/examine(mob/user)
	...
	. += span_notice("Космос безопасен, пока капюшон опущен.")
...
/obj/item/clothing/suit/hooded/cultrobes/void/proc/make_visible()
	...
	if(isliving(loc))
		...
		loc.visible_message(span_notice("Калейдоскоп цветов рассыпается вокру [loc], и внезапно появляется надетый плащ!"))
