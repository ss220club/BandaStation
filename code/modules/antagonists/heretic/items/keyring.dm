...
/obj/item/card/id/advanced/heretic/examine(mob/user)
	...
	. += span_hypnophrase("Используйте на ней ID-карту или её на карте, чтобы поглотить ID-карту и скопировать доступа.")
	...
	. += span_hypnophrase("<b>Ctrl-Клик по карте</b> заставит карту создавать инвертированную версию порталов, которые телепортируют вас случайно, а язычников - от одной двери к другой.")
