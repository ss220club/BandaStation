...
/obj/item/clothing/neck/heretic_focus/moon_amulet/proc/channel_amulet(mob/user, atom/target)
	...
	if(IS_HERETIC_OR_MONSTER(human_target))
		living_user.balloon_alert(living_user, "сопротивляется эффектам!")
		return FALSE
	...
	if(human_target.mob_mood.sanity_level < SANITY_LEVEL_UNSTABLE)
		living_user.balloon_alert(living_user, "их разум слишком крепок!")
		...
		living_user.balloon_alert(living_user, "их разум поддается и открывается правде!")
