...
/obj/item/clothing/neck/heretic_focus/moon_amulet/proc/modify_attack_message(obj/item/weapon, mob/living/victim, mob/living/attacker)
	...
	var/list/attack_list = list(
		"Ты махаешь [weapon] в сторону [victim], разрезая образ [victim.p_Their()] пополам.",
		"Ты наносишь удар  [victim] с помощью [weapon], изнутри льется каскад. Безупречно",
		"Когда оно укусит глубоко, твое [weapon] с [victim] лишние мысли.",
	)
	to_chat(attacker, span_danger(pick(attack_list)))

	var/list/victim_list = list(
		"Тебя поражает [attacker], но [weapon] разрывает не только твое тело, но и что-то большее.",
		"ты видишь дугу света, когда [weapon] в руках [attacker] поворачивается к тебе, и на мгновение окружающий мир видится тетрахромном спектре",
		"Когда [attacker] режет тебя [weapon], ты теряешь что-то глубокое внутри. Эта агония сильней любой раны",
	)
