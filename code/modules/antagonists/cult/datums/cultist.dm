...
/datum/antagonist/cult/proc/make_cult_leader()
	...
	to_chat(owner.current, span_cult_large("<span class='warningplain'>Вы - Мастер культа</span>. \
		Как Мастер культа у вас громче голос при коммуникации и можете помечать цели, как локацию и не культиста, \
		чтобы направить культ туда. И вы можете призывать живых культистов к вашей локации <b><i>единожды</i></b>.\
