...
/datum/mood_event/besthug/add_effects(mob/friend)
	...
	description = "[friend.name] весело находиться рядом с [friend.declent_ru(INSTRUMENTAL)], не могу нарадоваться!"
...
/datum/mood_event/pet_animal/add_effects(mob/animal)
	...
	if(HAS_PERSONALITY(owner, /datum/personality/animal_disliker))
		mood_change = -1
		description = "Беееее, [animal.name] такой грязный! Не буду его трогать от греха подальше!"
		return
	...
	if(dog_fan && iscat)
		mood_change = -1
		description = "Я не люблю [animal.name]! Я люблю собак!"
		return
	if(cat_fan && isdog)
		mood_change = -1
		description = "Я не люблю [animal.name]! Я люблю котов!"
		return

	mood_change = 1
	description = "[animal.name] такой милый!"
...
/datum/mood_event/slots/add_effects()
	if(HAS_PERSONALITY(owner, /datum/personality/gambler))
		...
		mood_change *= -1
		description = "И почему я теряю свою деньги и время на казино?"


/datum/mood_event/slots/win
	description = "Ай краса, я выйграл!"
	mood_change = 2
	timeout = 5 MINUTES
	event_flags = MOOD_EVENT_GAMING
...
/datum/mood_event/slots/win/jackpot
	description = "ЖЕЕЕЕКПОООТ! УААААААА!"
	mood_change = 6
	timeout = 30 MINUTES
