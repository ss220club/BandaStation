...
/datum/martial_art/cqc/proc/Slam(mob/living/attacker, mob/living/defender)
	...
	defender.visible_message(
		...
		span_userdanger("[capitalize(attacker)] швырнул вас на землю!"),
...
/datum/martial_art/cqc/proc/Kick(mob/living/attacker, mob/living/defender)
	...
	if(defender.body_position == LYING_DOWN && !defender.IsUnconscious() && defender.get_stamina_loss() >= 100)
		...
		defender.visible_message(
			span_danger("[capitalize(attacker.declent_ru(NOMINATIVE))] отбрасывает [defender.declent_ru(GENITIVE)] назад!"),
			span_userdanger("Вы были оьброшены [attacker.declent_ru(INSTRUMENTAL)]!"),
			span_hear("Вы слышите противный звук удара плоти о плоть!"),
			COMBAT_MESSAGE_RANGE,
			attacker,
		)
		to_chat(attacker, span_danger("Вы отбрасываете [defender.declent_ru(ACCUSATIVE)] back!"))
...
/datum/martial_art/cqc/proc/Consecutive(mob/living/attacker, mob/living/defender)
	...
	defender.visible_message(
		span_danger("[attacker.declent_ru(NOMINATIVE)] последовательно наносит удары в живот, шею и спину [defender.declent_ru(GENITIVE)]."), \
...
/datum/martial_art/cqc/disarm_act(mob/living/attacker, mob/living/defender)
	...
	if(prob(65) && (defender.stat == CONSCIOUS || !defender.IsParalyzed() || !restraining_mob?.resolve()))
		...
		to_chat(attacker, span_danger("Вы бьёте в челюсть [defender.declent_ru(GENITIVE)],[disarmed_item ? " выбивая из [defender.ru_p_them()] рук [disarmed_item] и" : ""] оставляя [defender.ru_p_them()] дезориентированным!"))
...
/mob/living/proc/CQC_help()
	...
	to_chat(usr, "[span_notice("CQC пинок")]: Удар Удар . Отбросьте оппонента от себя. Отбрасывание оглушённого оппонента наносит урон выносливости.")
	to_chat(usr, "[span_notice("Сдерживание")]: Захват Захват. Удерживает в захвате и обезоруживает оппонета, чтобы вырубить его удушающим приемом.")
