...
/datum/martial_art/the_sleeping_carp/proc/wrist_wrench(mob/living/attacker, mob/living/defender)
	...
	defender.visible_message(
		span_danger("[capitalize(attacker.declent_ru(NOMINATIVE))] яростно выкручивает [affecting.declent_ru(NOMINATIVE)] [defender.declent_ru(GENITIVE)]!"),
		span_userdanger("[capitalize(attacker.declent_ru(NOMINATIVE))] яростно выкручивает [affecting.declent_ru(ACCUSATIVE)]!"),
		span_hear("Вы слышите мерзкий звук ломающейся кости!"), null, attacker)
	to_chat(attacker, span_danger("Вы жестоко выкручиваете [affecting.declent_ru(NOMINATIVE)] [defender.declent_ru(GENITIVE)]!"))
...
/mob/living/proc/sleeping_carp_help()
	...
	sНекоторые виды оружия ближнего боя, такие как арбалеты, копья, короткие клинки, ножи, ящики с инструментами, бейсбольные биты и неблокируемые малые объекты можно носить, и их наличие не скажется на вашу способность к блокированию. Используйте это для получения тактического преимущества. \n\
	Кроме того, вы более устойчивы к ранениям в бою, и ваши конечности не могут отделиться от вашего тела. Это даёт вам дополнительную выдержку,особенно в затяжном бою против рубящего и подобного урожия, вызывающего кровотечение. \n\
...
/obj/item/clothing/gloves/the_sleeping_carp
	name = "carp gloves"
	desc = "Эти перчатки способны наделить человека способностями для использовать Спящего Карпа."
	icon_state = "black"
