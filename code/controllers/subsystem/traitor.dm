...
/datum/controller/subsystem/traitor/Initialize()
	//  BANDASTATION EDIT START: current_progression_scaling = 1 MINUTES * CONFIG_GET(number/traitor_scaling_multiplier)
...
/datum/controller/subsystem/traitor/fire(resumed)
	//  BANDASTATION EDIT START: var/previous_progression = current_global_progressionvar/previous_progression = current_global_progression/ current_global_progression = (STATION_TIME_PASSED()) * CONFIG_GET(number/traitor_scaling_multiplier) / var/progression_increment = current_global_progression - previous_progression
	...
	for(var/datum/uplink_handler/handler in uplink_handlers)
		...
		var/deviance = (previous_global_progression - handler.progression_points) / progression_scaling_deviance // BANDASTATION EDIT: if(handler.progression_points < current_global_progression)
		if(abs(deviance) < 0.01) // BANDASTATION EDIT: // If we got unsynced somehow, just set them to the current global progression
