...
/atom/proc/Initialize(mapload, ...)
	...
	if(((plane - GAME_PLANE) % PLANE_RANGE == 0) && should_spawn_los_proxy(src) && !render_target) // modulo check to handle multiz offsets without relying on SSmapping tables
		render_target = "rt_\ref[src]"
		var/atom/movable/los_proxy/proxy = new(null)
		proxy.attach_to(src)
		proxy.alpha = src.alpha
