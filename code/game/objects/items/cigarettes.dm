...
/obj/item/match/proc/matchignite()
	if(lit || burnt || broken)
		return
// BANDASTATION ADDITION START: Pollution
	var/turf/my_turf = get_turf(src)
	my_turf.pollute_turf(/datum/pollutant/smoke/sulphur, 5)
// BANDASTATION ADDITION END: Pollution
...
/obj/item/cigarette
	...
	var/pollution_type = /datum/pollutant/smoke // BANDASTATION ADDITION: Pollution
...
/obj/item/cigarette/process(seconds_per_tick)
	...
	var/turf/location = get_turf(src) // BANDASTATION ADDITION: Pollution
	user?.ignite_mob()
	...
	location.pollute_turf(pollution_type, 3, POLLUTION_PASSIVE_EMITTER_CAP) // BANDASTATION ADDITION: Pollution
