...
/datum/action/item_action/stealth_mode
	...
	var/usage_per_tick = 1.5 SECONDS
...
/datum/action/item_action/stealth_mode/process(seconds_per_tick)
	if(!stealth_engaged)
		// Recharge over time
		charge = min(max_charge, charge + (usage_per_tick * 2) * seconds_per_tick) // BANDASTATION EDIT: stealth mode refactor
		build_all_button_icons(UPDATE_BUTTON_STATUS)
		return
	...
	if(our_turf?.check_lumcount_above(0.3))
		...
		charge = max(0, charge - usage_per_tick * seconds_per_tick) // BANDASTATION EDIT: stealth mode refactor
		build_all_button_icons(UPDATE_BUTTON_STATUS)

	else if(recharge_while_active)
		// Return charage while invisible + in the darkness + recharge_while_active
		charge = min(max_charge, charge + (usage_per_tick * 1.5) * seconds_per_tick) // BANDASTATION EDIT: stealth mode refactor
		build_all_button_icons(UPDATE_BUTTON_STATUS)
...
/datum/action/item_action/stealth_mode/weaker
	...
	usage_per_tick = 0.75 SECONDS // BANDASTATION EDIT: stealth mode refactor
	min_alpha = 20
