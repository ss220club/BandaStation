...
/proc/healthscan(mob/user, mob/living/target, mode = SCANNER_VERBOSE, advanced = FALSE, tochat = TRUE)
	...
	if(!has_brain) // kept exclusively for soul purposes
		render_list += "<span class='alert ml-1'>Субъект демонстрирует признаки умственной отсталости.</span><br>"

	if(iscarbon(target))
		var/mob/living/carbon/carbontarget = target
		if(LAZYLEN(carbontarget.quirks))
			render_list += "<span class='info ml-1'>Крупные отклонения субъекта: [carbontarget.get_quirk_string(FALSE, CAT_QUIRK_MAJOR_DISABILITY, from_scan = TRUE)].</span><br>"
			if(advanced)
	...
	if(iscarbon(target))
		...
		if(any_damage || (mode == SCANNER_VERBOSE && (any_missing || any_wounded || any_embeds)))
			...
			var/dmgreport = "<span class='info ml-1'>Состояние тела:</span>\
							...
							<td style='width:5em;'><font color='#ff3333'><b>Физический</b></font></td>\
	...
	if(blood_alcohol_content > 0)
		if(blood_alcohol_content >= 0.24)
			...
			render_list += "<span class='alert ml-1'>[blood_type?.get_blood_name() || "Уровень"] алкоголя в крови: <b>КРИТИЧЕСКИЙ [blood_alcohol_content]%</b></span><br>"
		else
			render_list += "<span class='info ml-1'>[blood_type?.get_blood_name() || "Уровень"] алкоголя в крови: [blood_alcohol_content]%</span><br>"
