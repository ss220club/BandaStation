...
/obj/item/stack/medical/proc/try_heal(mob/living/patient, mob/living/user, healed_zone, silent = FALSE, auto_change_zone = TRUE, continuous = FALSE)
	...
	if(!iscarbon(patient))
		...
		patient.balloon_alert(user, "обработано полностью")
...
/obj/item/stack/medical/gauze
	name = "medical gauze"
	desc = "Рулон эластичной ткани, идеально подходящий для заживления всех видов ран: от порезов и ожогов до сломанных костей. "
	gender = PLURAL
...
/obj/item/stack/medical/gauze/add_item_context(obj/item/source, list/context, atom/target, mob/living/user)
	if(iscarbon(target))
		context[SCREENTIP_CONTEXT_LMB] = "Наложить повязку"
		return CONTEXTUAL_SCREENTIP_SET
	return NONE
...
/obj/item/stack/medical/gauze/try_heal_checks(mob/living/patient, mob/living/user, healed_zone, silent = FALSE)
	...
	if(isnull(limb))
		if(!silent)
			patient.balloon_alert(user, "отсутствует [parse_zone(healed_zone, NOMINATIVE)]!")
		return FALSE
	if(!LAZYLEN(limb.wounds))
		if(!silent)
			patient.balloon_alert(user, "нет ран!") // good problem to have imo
		return FALSE
	if(limb.current_gauze && (limb.current_gauze.absorption_capacity * 1.2 > absorption_capacity)) // ignore if our new wrap is < 20% better than the current one, so someone doesn't bandage it 5 times in a row
		if(!silent)
			patient.balloon_alert(user, pick("уже перевязано!", "повязка чистая!")) // good enough
		return FALSE
	for(var/datum/wound/woundies as anything in limb.wounds)
		if(woundies.wound_flags & ACCEPTS_GAUZE)
			return TRUE
	if(!silent)
		patient.balloon_alert(user, "не перевязать!")
	return FALSE
...
/obj/item/stack/medical/gauze/try_heal(mob/living/patient, mob/living/user, healed_zone, silent, auto_change_zone, continuous)
	...
	if(any_scanned)
		...
		if(user == patient)
			if(!silent)
				user.visible_message(
					span_warning("[user] начинает профессионально перевязывать раны на [limb.ru_plaintext_zone[PREPOSITIONAL]] при помощи [src.declent_ru(GENITIVE)]..."),
					span_warning("Вы начинаете быстро перевязывать раны на [limb.ru_plaintext_zone[PREPOSITIONAL]] при помощи [src.declent_ru(GENITIVE)], следуя голографическим указаниям..."),
					visible_message_flags = ALWAYS_SHOW_SELF_MESSAGE,
				)
		else
			if(!silent)
				user.visible_message(
					span_warning("[user] начинает профессионально перевязывать раны на [limb.ru_plaintext_zone[PREPOSITIONAL]] [patient.declent_ru(DATIVE)] при помощи [src.declent_ru(GENITIVE)]..."),
					span_warning("Вы начинаете быстро перевязывать раны на [limb.ru_plaintext_zone[PREPOSITIONAL]] [patient.declent_ru(DATIVE)] при помощи [src.declent_ru(GENITIVE)], следуя голографическим указаниям..."),
					visible_message_flags = ALWAYS_SHOW_SELF_MESSAGE,
				)
	else
		if(!silent)
			user.visible_message(
				span_warning("[user] начинает перевязывать раны на [patient.declent_ru(ACCUSATIVE)] [limb.ru_plaintext_zone[PREPOSITIONAL]] при помощи [src.declent_ru(GENITIVE)]..."),
				span_warning("Вы начинаете перевязывать раны на [user == patient ? "своей [limb.ru_plaintext_zone[PREPOSITIONAL]]" : "[limb.ru_plaintext_zone[PREPOSITIONAL]] [patient.declent_ru(DATIVE)]"] при помощи [src.declent_ru(GENITIVE)]..."),
				visible_message_flags = ALWAYS_SHOW_SELF_MESSAGE,
			)
	...
	if(!silent)
		patient.balloon_alert(user, "перевязана [parse_zone(healed_zone, NOMINATIVE)]")
		user.visible_message(
			span_green("[user] накладывает [src.declent_ru(ACCUSATIVE)] на [limb.ru_plaintext_zone[PREPOSITIONAL]] [patient.declent_ru(DATIVE)]."),
			span_green("Вы перевязываете раны на [user == patient ? "своей [limb.ru_plaintext_zone[PREPOSITIONAL]]" : "[limb.ru_plaintext_zone[PREPOSITIONAL]] [patient.declent_ru(DATIVE)]"]."),
			visible_message_flags = ALWAYS_SHOW_SELF_MESSAGE,
		)
		if(heal_end_sound)
			playsound(patient, heal_end_sound, 75, TRUE, MEDIUM_RANGE_SOUND_EXTRARANGE)
...
/obj/item/stack/medical/gauze/attackby(obj/item/I, mob/user, list/modifiers, list/attack_modifiers)
	if(I.tool_behaviour == TOOL_WIRECUTTER || I.get_sharpness())
		...
		if(IsReachableBy(user))
			user.visible_message(span_notice("[user] разрезает [src.declent_ru(ACCUSATIVE)] в куски ткани с помощью [I.declent_ru(GENITIVE)]."), \
				span_notice("Вы разрезаете [src.declent_ru(ACCUSATIVE)] в куски ткани с помощью [I.declent_ru(GENITIVE)]."), \
				span_hear("Вы слышите, как кто-то режет."))
		else //telekinesis
			visible_message(span_notice("[capitalize(I.declent_ru(NOMINATIVE))] разрезает [src.declent_ru(ACCUSATIVE)] в куски ткани."), \
...
/obj/item/stack/medical/gauze/suicide_act(mob/living/user)
	user.visible_message(span_suicide("[user.declent_ru(NOMINATIVE)] начинает затягивать [src.declent_ru(ACCUSATIVE)] вокруг своей шеи! Кажется, [user.ru_p_they()] забыл, как пользоваться медикаментами!"))
	return OXYLOSS
