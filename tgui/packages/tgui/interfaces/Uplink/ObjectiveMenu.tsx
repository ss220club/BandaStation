                          ...
                          >
                            <Stack textAlign="center" fill align="center">
                              <Stack.Item textAlign="center" width="100%">
                                Пустая цель, перекиньте цель чтобы взятся за неё
                              </Stack.Item>
                            </Stack>
                          </Box>
                        </Stack.Item>
                ...
                {(maximumPotentialObjectives === 0 && (
                  <Dimmer>
                    ...
                    <Box color="red" fontSize={3}>
                      Вы заблокированы от выполнения задач.
                    </Box>
                  </Dimmer>
                )) ||
...
export const ObjectiveElement = (props: ObjectiveElementProps) => {
  ...
  switch (objectiveState) {
    case ObjectiveState.Invalid:
      objectiveCompletionText = 'аннулированно';
      break;
    case ObjectiveState.Completed:
      objectiveCompletionText = 'Выполнено';
      break;
    case ObjectiveState.Failed:
      objectiveCompletionText = 'Провалено';
      break;
  }
    ...
    <Flex height={grow ? '100%' : undefined} direction="column">
      ...
      <Flex.Item grow={grow} basis="content">
        <Box className="UplinkObjective__Content" height="100%">
          ...
          {!hideTcRep && (
            <Box mt={1}>
              Провал этой цели повлечёт штраф в размере {telecrystalPenalty} ТК.
            </Box>
          )}
          {finalObjective && objectiveState === ObjectiveState.Inactive && (
            <NoticeBox mt={1}>
              Взятие этой цели заблокирует возможность взять другие цели! Также
              вы не сможете отменить эту цель.
            </NoticeBox>
          )}
        </Box>
      </Flex.Item>
              ...
              <Stack.Item>
                <Stack align="center" justify="center">
                  ...
                  >
                    ...
                    <Box ml={1} as="span">
                      {calculateProgression(progressionReward)} Уровень угрозы
                        ...
                        <Tooltip
                          content={
                            <Box>
                              ...
                              {progressionDiff > 0 ? 'меньше' : 'больше'} угрозы
                              за эту цель. Это потому что ваш уровень{' '}
                              {progressionDiff > 0 ? 'выше ' : 'ниже '}
