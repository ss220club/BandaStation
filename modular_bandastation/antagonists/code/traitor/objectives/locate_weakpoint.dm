...
/datum/traitor_objective/locate_weakpoint
	name = "Определите уязвимую точку станции и разместите взрывное устройство поблизости для её подрыва."
...
/datum/traitor_objective/locate_weakpoint/generate_ui_buttons(mob/user)
	...
	if(!locator_sent)
		buttons += add_ui_button("", "Нажатие этой кнопки материализует в вашей руке локатор уязвимых точек", "globe", "locator")
	if(weakpoint_found && !bomb_sent)
		buttons += add_ui_button("", "Нажатие этой кнопки материализует в ваших руках взрывчатку ES8.", "bomb", "shatter_charge")
	return buttons
...
/datum/traitor_objective/locate_weakpoint/ui_perform_action(mob/living/user, action)
	...
	switch(action)
		if("locator")
			...
			locator.balloon_alert(user, "локатор материализуется в вашей руке")
...
/datum/traitor_objective/locate_weakpoint/proc/create_shockwave(center_x, center_y, center_z)
	...
	priority_announce(
				"Внимание, экипаж! Похоже, в слабом месте вашей станции была взорвана мощная бомба, вызвавшая серьёзные структурные повреждения.",
...
/obj/item/weakpoint_locator
	...
	desc = "Устройство, способное определять слабые места станции путём триангуляции. Его нужно использовать в %AREA1% и %AREA2%, чтобы определить слабое место. Внимание: как только процесс начнётся, станционный ИИ будет уведомлен!"
...
/obj/item/weakpoint_locator/attack_self(mob/living/user, modifiers)
	...
	if(!objective || objective.objective_state == OBJECTIVE_STATE_INACTIVE)
		to_chat(user, span_warning("Твоё время использовать [src] ещё не наступило."))
		return

	if(objective.handler.owner != user.mind)
		to_chat(user, span_warning("Ты в душе не чаешь как использовать [src]."))
		return

	var/area/user_area = get_area(user)
	if(!(user_area.type in objective.scan_areas))
		balloon_alert(user, "не та зона!")
		playsound(user, 'sound/machines/buzz/buzz-sigh.ogg', 30, TRUE)
		return

	if(!objective.scan_areas[user_area.type])
		balloon_alert(user, "уже просканированно!")
		playsound(user, 'sound/machines/buzz/buzz-sigh.ogg', 30, TRUE)
		return

	user.visible_message(span_danger("[user] нажимает несколько кнопок на [src], и оно начинает зловеще пищать!"), span_notice("Ты активируешь [src] и начинаешь сканировать зону. Не покидай [get_area_name(user, TRUE)] до конца анализа!"))
	...
	for(var/area/scan_area as anything in objective.scan_areas)
		if(objective.scan_areas[scan_area])
			say("Следущая зона для анализа это [initial(scan_area.name)]")
			return

	to_chat(user, span_notice("Анализ завершён. Стукрутрная слабая точка обнаружена в [initial(objective.weakpoint_area.name)]."))
...
/obj/item/grenade/c4/es8
	...
	desc = "Высокомощная взрывчатка, предназначенный для создания ударной волны в слабом месте станции."
...
/obj/item/grenade/c4/es8/plant_c4(atom/bomb_target, mob/living/user)
	if(!IS_TRAITOR(user))
		to_chat(user, span_warning("Вы не понимаете как активировать [src]."))
		return FALSE

	var/datum/traitor_objective/locate_weakpoint/objective = objective_weakref.resolve()
	if(!objective || objective.objective_state == OBJECTIVE_STATE_INACTIVE || objective.handler.owner != user.mind)
		to_chat(user, span_warning("Вы думаете что активировать [src] это крайне не плохая идея."))
		return FALSE

	var/area/target_area = get_area(bomb_target)
	if (target_area.type != objective.weakpoint_area)
		to_chat(user, span_warning("[src] может быть подорвана только [initial(objective.weakpoint_area.name)]."))
		return FALSE

	if(!isfloorturf(bomb_target) && !iswallturf(bomb_target))
		to_chat(user, span_warning("[src] можно разместить только на полу или стене!"))
		return FALSE
...
/obj/item/grenade/c4/es8/detonate(mob/living/lanced_by)
	...
	if (target_area.type != objective.weakpoint_area)
		...
		new_bomb.balloon_alert_to_viewers("Не то место!")
