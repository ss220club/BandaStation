...
/datum/traitor_objective/sabotage_machinery
	...
	description = "Абстрактная цель которая не должна быть в аплинке."
...
/datum/traitor_objective/sabotage_machinery/destroy
	...
	description = "Уничтожьте %MACHINE% чтобы навредить работе %JOB% отдела."
...
/datum/traitor_objective/sabotage_machinery/trap
	...
	description = "Уничтожьте %MACHINE% чтобы навредить работе %JOB% отдела. Если вы сможете заминировать машинерию и кто-то на ней подорвётся, то вы получите %PROGRESSION% бонусной репутации и %TC% телекристалов."
...
/datum/traitor_objective/sabotage_machinery/trap/generate_ui_buttons(mob/user)
	...
	if(!traitor_trapper_given)
		buttons += add_ui_button("", "Нажатие этой кнопки материализует взрывную ловушку в вашей руке, эту ловушку вы можете установить на цель для саботажа", "wifi", "summon_gear")
	return buttons
...
/obj/item/traitor_machine_trapper
	...
	desc = "Это выглядит КРАЙНЕ подозрительно."
...
/obj/item/traitor_machine_trapper/examine(mob/user)
	...
	if(target_machine_path)
		. += span_notice("Устройство можно распложить с помощью <b>клика по [initial(target_machine_path.name)]</b>. Ловушку можно убрать отвёрткой.")
	. += span_notice("Помните, вы вероятней всего оставите отпечатки пальцев на ловушке. Наденьте <b>перчатки</b> для вашей же безопасности!")

/obj/item/traitor_machine_trapper/pre_attack(atom/target, mob/living/user, params)
	. = ..()
	if (. || !istype(target, target_machine_path))
		return
	balloon_alert(user, "устанавлеваем...")
