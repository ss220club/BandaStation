...
/datum/traitor_objective/target_player/kidnapping/generate_ui_buttons(mob/user)
	...
	if(!pod_called)
		buttons += add_ui_button("Вызвать капсулу", "Нажатие этой кнопки вызовет капсулу для транспортировки цели.", "rocket", "call_pod")
	return buttons
	...
	switch(action)
		if("call_pod")
			...
			if(user_area.type != dropoff_area)
				to_chat(user, span_warning("Вы должны находится в [initial(dropoff_area.name)] чтобы вызвать капсулу."))
				return

			if(target_area.type != dropoff_area)
				to_chat(user, span_warning("[target.real_name] должен быть в [initial(dropoff_area.name)] чтобы вы могли вызвать капсулу."))
				return
