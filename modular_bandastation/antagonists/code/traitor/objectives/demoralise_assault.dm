...
/datum/traitor_objective/target_player/assault
	name = "Нападите на %TARGET% в должности %JOB TITLE%"
	description = "%TARGET% опеределён как потенциально будущий агент. \
		Приподайте ему хорощую збучку. \
		%COUNT% ударов хватит чтобы понизить их моральных дух. \
		Не убивайте цель, мы в будущем хотим её завербовать."
...
/datum/traitor_objective/target_player/assault/generate_ui_buttons(mob/user)
	...
	if(attacks_required > attacks_inflicted)
		buttons += add_ui_button("[attacks_required - attacks_inflicted]", "Показывает количество ударов которых необходимо нанести.", "hand-rock-o", "none")
	return buttons
