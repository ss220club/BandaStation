...
/datum/traitor_objective/target_player/infect
	name = "Заразите %TARGET% в должности %JOB TITLE%"
	description = "Заразите цель эксперментальным вирусом EHMS."
...
/datum/traitor_objective/target_player/infect/generate_ui_buttons(mob/user)
	...
	if(!injector_given)
		buttons += add_ui_button("", "Нажатие кнопки материализует инжектор с EHMS в вашей руке, вы должны вколоть вирус в цель.", "syringe", "summon_pen")
	return buttons
...
/datum/traitor_objective/target_player/infect/ui_perform_action(mob/living/user, action)
	...
	switch(action)
		if("summon_pen")
			...
			ehms.balloon_alert(user, "инжектор материализуется в вашей руке")
...
/obj/item/reagent_containers/hypospray/medipen/manifoldinjector
	...
	desc = "Экспериментальный инжектор с EHMS."
	icon_state = "tbpen"
...
/obj/item/reagent_containers/hypospray/medipen/manifoldinjector/attack(mob/living/affected_mob, mob/living/carbon/human/user)
	if(used)
		return ..()
	to_chat(affected_mob, span_warning("Вы чуствуете как кто-то пытается что-то вам вколоть."))
	balloon_alert(user, "Вкалываем...")
	log_combat(user, affected_mob, "попытка вколоть", src)
