...
/datum/traitor_objective/target_player/eyesnatching/generate_ui_buttons(mob/user)
	...
	if(!spawned_eyesnatcher)
		buttons += add_ui_button("", "Нажатие этой кнопки материализует устройство, которое может удалить глаза у свящего или обездвиженного человека.", "syringe", "eyesnatcher")
	return buttons
...
/datum/traitor_objective/target_player/eyesnatching/ui_perform_action(mob/living/user, action)
	...
	switch(action)
		if("eyesnatcher")
			...
			eyesnatcher.balloon_alert(user, "устройство матерализуется в твоей руке")

/obj/item/eyesnatcher
	name = "portable eyeball extractor"
	desc = "Чрезмерно сложное устройство, оно способно пробить череп цели и извлечь из него глазные яблоки, если приложить достаточную силу."
...
/obj/item/eyesnatcher/attack(mob/living/carbon/human/target, mob/living/user, params)
	...
	target.visible_message(
		span_warning("[user] приставляет [src] к черепу [target]!"),
		span_userdanger("[user] приставляет [src] к вашему черепу!"))
	...
	to_chat(target, span_userdanger("Вы ощущаете, как что-то надавливает на ваш череп!"))
	...
	target.visible_message(
		span_danger("[src] пробивает череп [target], и крайне мерзко извлекает глаза!"),
		span_userdanger("Что-то пробивает ваш череп, вы чуствуете ужасную боль! БЛЯЯЯТЬ!"),
		span_hear("Вы слышите отвратительный звук пронзающейся металлом плоти!")
	)
	eyeballies.apply_organ_damage(eyeballies.maxHealth)
	target.emote("scream")
	playsound(target, 'sound/effects/wounds/crackandbleed.ogg', 100)
	log_combat(user, target, "пробил череп (eye snatching)", src)

	if(!do_after(user, eye_snatch_enthusiasm, target = target, extra_checks = CALLBACK(src, PROC_REF(eyeballs_exist), eyeballies, head, target)))
		return

	if(!target.is_blind())
		to_chat(target, span_userdanger("Вы ослепли!"))
	if(prob(1))
		to_chat(target, span_notice("По крайней мере вы получили прикид пирата..."))
