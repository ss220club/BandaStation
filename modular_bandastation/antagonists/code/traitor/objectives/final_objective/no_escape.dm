/datum/traitor_objective/ultimate/no_escape
	name = "Прикрепите маяк к эвакуационному шаттлу, который привлечёт сингулярность, которая поглатит этот самый шаттл."
	description = "Отправляйтесь в %AREA%, и получите маяк. Установите маяк в любой точке шаттла, \
	и зарядите его с помощью индуктора, ОНО ГРЯДЁТ. Внимание: сингулярность уничтожин всё на своём пути, включая вас."
...
/datum/traitor_objective/ultimate/no_escape/generate_ui_buttons(mob/user)
	...
	if(!sent_beacon)
		buttons += add_ui_button("", "Нажатие на кнопку вызовет капсулу с маяком.", "beacon", "beacon")
	return buttons
	...
	switch(action)
		if("beacon")
			...
			if(delivery_area.type != beacon_spawn_area_type)
				to_chat(user, span_warning("Вы должны находится в [initial(beacon_spawn_area_type.name)] чтобы получить маяк."))
				return
			sent_beacon = TRUE
			podspawn(list(
