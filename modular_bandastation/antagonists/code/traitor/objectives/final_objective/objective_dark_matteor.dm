...
/obj/item/paper/dark_matteor_summoning
	...
	default_raw_text = {"
		...
		<br>
		<br>
		Оперативник, этот ящик содержит 10+1(1 запасной) спуников против метеоритов, украденных с логистической точки НТ. Ваша миссия заключается
		в том чтобы вы распложили спутники рядом со станцией, а также перенастроить их своим специальным для этого ЕМАГом. Важно: ЕМАГ уходит на перезарядку
		на 30 секунд после каждой настройки, также НТ зафиксирует странные сигналы которые издают перенастроенные спутники. Это значит
		у вас есть 5 минут для тихой работы, и 1 минута после того как об вас сообщит ЦК НТ.<br>
		<br>
		Это крайне опасная операция. Вам будет нужна удача, оружие и подготовка. А какая награда?
