...
/datum/traitor_objective/destroy_item/demoralise
	description = "Найдите %ITEM% и уничтожьте его любым способом. \
		Мы верим что этот дорогой предмет важен для морали экипажа станции. \
		Уничтожение этого предмета поможет в нашей рекрутинговой компании."
...
/datum/traitor_objective/destroy_item/generate_objective(datum/mind/generating_for, list/possible_duplicates)
	...
	if(target_item.destruction_method != null)
		...
		replace_in_name("%METHOD%", "Этот предмет может быть уничтожен любым способом, например переработчиком который можно найти в мусорке.")
...
/datum/traitor_objective/destroy_item/generate_ui_buttons(mob/user)
	...
	if(special_equipment)
		buttons += add_ui_button("", "Нажатие этой кнопки призовёт дополнительное снаряжение которое нужно для выполнения миссии.", "tools", "summon_gear")
	return buttons
...
/datum/traitor_objective/destroy_item/ui_perform_action(mob/living/user, action)
	...
	switch(action)
		if("summon_gear")
			...
			user.balloon_alert(user, "снаряжение материализуется в вашей руке")
