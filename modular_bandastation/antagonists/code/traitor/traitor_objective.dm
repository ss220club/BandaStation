...
/datum/traitor_objective/proc/fail_objective(penalty_cost = 0, trigger_update = TRUE)
	...
	handle_cleanup()
	log_traitor("[key_name(handler.owner)] [objective_state == OBJECTIVE_STATE_INACTIVE? "missed" : "провалена"] [to_debug_string()]")
	if(penalty_cost)
...
/datum/traitor_objective/proc/succeed_objective()
	...
	handle_cleanup()
	log_traitor("[key_name(handler.owner)] [objective_state == OBJECTIVE_STATE_INACTIVE? "missed" : "выполнена"] [to_debug_string()]")
