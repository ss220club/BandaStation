...
/obj/item/stake
	...
	icon = 'modular_bandastation/antagonists/code/bloodsuckers_220/icons/bloodsuckers/stakes.dmi'
	icon_state = "wood"
	inhand_icon_state = "wood"
	lefthand_file = 'modular_bandastation/antagonists/code/bloodsuckers_220/icons/bloodsuckers/bs_leftinhand.dmi'
	righthand_file = 'modular_bandastation/antagonists/code/bloodsuckers_220/icons/bloodsuckers/bs_rightinhand.dmi'
...
/obj/item/book/kindred/afterattack(mob/living/target, mob/living/user, flag, params)
	...
	if(!user.can_read(src) || in_use || (target == user) || !ismob(target))
		return
	if(!HAS_MIND_TRAIT(user, TRAIT_BLOODSUCKER_HUNTER))
...
/obj/item/book/kindred/attack_self(mob/living/user)
	if(user.mind && !HAS_MIND_TRAIT(user, TRAIT_BLOODSUCKER_HUNTER))
		if(IS_BLOODSUCKER(user))
			to_chat(user, span_warning("[src] burns your hands as you try to use it!"))
			user.apply_damage(3, BURN, pick(BODY_ZONE_L_ARM, BODY_ZONE_R_ARM))
		else
