...
/datum/controller/subsystem/sunlight/fire(resumed = FALSE)
	...
	if(sunlight_active)
		...
		if(time_til_cycle <= 1)
			...
			message_admins("BLOODSUCKER NOTICE: Daylight Ended. Resetting to night (Lasts for [time_til_cycle * 10])")
	...
	switch(time_til_cycle)
		if(TIME_BLOODSUCKER_DAY_WARN)
			...
			warn_daylight(
				...
				vampire_warning_message = span_danger("Solar flares will bombard the station with dangerous UV radiation in [TIME_BLOODSUCKER_DAY_WARN  * 10] minutes. <b>Prepare to seek cover in a coffin or closet.</b>"),
			)
		...
		if(NONE)
			...
			message_admins("BLOODSUCKER NOTICE: Daylight Beginning (Lasts for [TIME_BLOODSUCKER_DAY  * 10] minutes.)")
			warn_daylight(
				danger_level = DANGER_LEVEL_SOL_ROSE,
				vampire_warning_message = span_userdanger("Solar flares bombard the station with deadly UV light! Stay in cover for the next [TIME_BLOODSUCKER_DAY * 10] minutes or risk Final Death!"),
