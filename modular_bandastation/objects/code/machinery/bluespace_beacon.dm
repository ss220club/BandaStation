#define BEACON_POINTS_PER_W 0.5e-6
...
#define BEACON_PORTAL_CHANCE_MIN 7
#define BEACON_PORTAL_CHANCE_MAX 10
...
/datum/station_goal/bluespace_beacon/get_report()
	return list(
		"#### Маяк реакльности",
		"Данная станция находится в досягаемости зоны сдерживания особой блюспейс аномалии.",
		"Вам необходимо построить и зарядить экспериментальный маяк реальности на борту вашей станции.<br>",
		"Базовые компоненты доступны для доставки через через отдел снабжения.<br>",
		"Учтите, что устройство является экспериментальным, и при его работе возможны неожиданные явления.",
	).Join("\n")
