...
/obj/ratvar
	...
	icon = 'modular_bandastation/clock_cult/icons/obj/clock_cult/512x512.dmi'
	icon_state = "ratvar"
	anchored = TRUE
	density = FALSE
...
/obj/ratvar/Initialize(mapload)
	...
	sound_to_playing_players('modular_bandastation/clock_cult/sound/effects/ratvar_reveal.ogg', 100)
	...
	if(area)
		var/mutable_appearance/alert_overlay = mutable_appearance('modular_bandastation/clock_cult/icons/obj/clock_cult/clockwork_effects.dmi', "ratvar_alert")
		notify_ghosts(
		...
		if(get_dist(src, GLOB.cult_narsie) < 5)
			if(next_attack_tick < world.time)
				...
				if(prob(max(length(GLOB.main_clock_cult?.members)/2, 15)))
					sound_to_playing_players('modular_bandastation/clock_cult/sound/effects/demon_dies.ogg', 100)
...
/obj/machinery/computer/ratvar_act()
	if(!clockwork) // special_appearance not in BandaStation
		clockwork = TRUE
