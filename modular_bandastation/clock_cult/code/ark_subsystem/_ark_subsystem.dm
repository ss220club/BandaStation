...
PROCESSING_SUBSYSTEM_DEF(the_ark)
	...
	var/list/hallucination_pool = list()
...
/datum/controller/subsystem/processing/the_ark/Initialize()
	...
	hallucination_pool = list() // dimension_theme / hallucination subsystems not in BandaStation
...
/datum/controller/subsystem/processing/the_ark/proc/convert_area_turfs(area/converted_area, conversion_percent = 100, counter_override)
	...
	for(var/turf/turf_to_transform in turfs_to_transform)
		if(istype(turf_to_transform, /turf/closed/wall/clockwork) || istype(turf_to_transform, /turf/open/indestructible/reebe_flooring))
			continue
...
///convert a turf to our dimension theme (walls→clockwork, floors→reebe)
/datum/controller/subsystem/processing/the_ark/proc/do_turf_conversion(turf/converted_turf)
	if(QDELETED(src) || istype(converted_turf, /turf/closed/wall/clockwork) || istype(converted_turf, /turf/open/indestructible/reebe_flooring))
		return
	...
	if(istype(converted_turf, /turf/closed/wall))
		...
		converted_turf.ChangeTurf(/turf/closed/wall/clockwork)
	else if(istype(converted_turf, /turf/open/floor))
		new /obj/effect/temp_visual/ratvar/floor(converted_turf)
		converted_turf.ChangeTurf(/turf/open/indestructible/reebe_flooring, flags = CHANGETURF_INHERIT_AIR)
