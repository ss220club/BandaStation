...
/datum/controller/subsystem/processing/the_ark/proc/crystal_warp_minds()
	...
	if(prob(50) && length(hallucination_pool))
	...
	switch(rand(1, 3))
		...
		if(2) //force open an airlock and bolt it
			...
			if(!picked_airlock)
				return
			var/sanity = 0
			while(sanity < 100 && length(airlocks) && (!picked_airlock.hasPower() || !is_station_level(picked_airlock.z)))
		...
		if(3) //emag a random atom from our list of valid types
			var/list/valid_emag_targets = list(
				...
				/obj/machinery/research/anomaly_refinery,				/obj/machinery/computer/bsa_control,
				/obj/machinery/vending,				/obj/machinery/computer/operating,			)
