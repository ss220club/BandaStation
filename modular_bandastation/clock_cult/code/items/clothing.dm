...
/obj/item/clothing/suit/hooded/clockwork
	...
	icon = 'modular_bandastation/clock_cult/icons/obj/clock_cult/clockwork_garb.dmi'
...
/obj/item/clothing/suit/hooded/clockwork/Initialize(mapload)
	...
	// turf_checker component not in BandaStation; suit empowerment on brass tiles disabled

/obj/item/clothing/suit/hooded/clockwork/proc/set_empowered_state(checker, empowered)
	if(!empowered)
...
/obj/item/clothing/head/helmet/clockwork/Initialize(mapload)
	...
	AddElement(/datum/element/clockwork_pickup, ~(ITEM_SLOT_HANDS))
	// turf_checker component not in BandaStation; helmet empowerment on brass tiles disabled

/obj/item/clothing/head/helmet/clockwork/proc/set_empowered_state(checker, empowered)
	if(!empowered)
		set_armor(initial(armor_type))
		clothing_flags &= ~STOPSPRESSUREDAMAGE
		return
...
/obj/item/clothing/suit/hooded/clockwork/cloak/hit_reaction(mob/living/carbon/human/owner, atom/movable/hitby, attack_text, final_block_chance, damage, attack_type)
	if(is_empowered && shroud_active && prob(CLOAK_DODGE_CHANCE)) //we handle this just a biiiiit too different from parent to make simply using the vars be viable
		...
		playsound(src, 'sound/weapons/etherealmiss.ogg', BLOCK_SOUND_VOLUME, vary = TRUE)
		return TRUE
...
/obj/item/clothing/glasses/clockwork/wraith_spectacles/attack_self(mob/user, modifiers)
	...
	if(iscarbon(user))
		...
		carbon_user.update_sight() // head_update not in BandaStation; update_sight covers vision refresh
...
/obj/item/clothing/glasses/clockwork/wraith_spectacles/process(seconds_per_tick)
	if(enabled && wearer)
		...
		// adjustOrganLoss not in BandaStation; eye damage from spectacles skipped
		if(damaged_mobs[wearer])
			var/wearer_data = damaged_mobs[wearer]
	...
	for(var/mob_entry in damaged_mobs)
		...
		if(mob_data["timer"] >= SECONDS_FOR_EYE_HEAL)
			// adjustOrganLoss not in BandaStation; eye damage restoration skipped
