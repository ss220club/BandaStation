...
/obj/structure/destructible/clockwork/anchoring_crystal
	...
	icon = 'modular_bandastation/clock_cult/icons/obj/clock_cult/clockwork_objects.dmi'
	icon_state = "obelisk"
	break_message = span_warning("As the Anchoring Crystal shatters you swear you hear a faint scream.")
	break_sound = 'modular_bandastation/clock_cult/sound/effects/ark_damage.ogg'
...
/obj/structure/destructible/clockwork/anchoring_crystal/Initialize(mapload)
	...
	// get_area_edge_turfs() not available in BandaStation; skip adjacent area conversion
	...
	// SSshuttle.registerEvacBlocker not available in BandaStation
...
/obj/structure/destructible/clockwork/anchoring_crystal/Destroy()
	// SSshuttle.clearEvacBlocker not available in BandaStation
	return ..()
...
/obj/structure/destructible/clockwork/anchoring_crystal/process(seconds_per_tick)
	for(var/mob/living/affected_mob in crystal_area)
		if(IS_CLOCK(affected_mob))
			affected_mob.adjust_tox_loss(-2.5 * seconds_per_tick, TRUE, TRUE) //slightly better tox healing around the crystal for servants
			continue
		affected_mob.adjust_silence_up_to(5 SECONDS * seconds_per_tick, 30 SECONDS)
		// adjust_emote_mute_up_to not available in BandaStation
...
/obj/structure/destructible/clockwork/anchoring_crystal/proc/on_update_overlays(atom/crystal, list/overlays)
	...
	var/mutable_appearance/shield_appearance = mutable_appearance('modular_bandastation/clock_cult/icons/obj/clock_cult/clockwork_effects.dmi', \
