...
/datum/admin_verb/change_sec_level/__avd_do_verb(client/user)
	. = ..()
	if(SSsecurity_level.get_current_level_as_number() == SEC_LEVEL_GAMMA && SSshuttle.getDock("gamma_home"))
		if(tgui_alert(user, "Желаете направить оружейный шаттл Гамма? Если не уверены, его можно будет направить позже в меню Secrets (Helpful -> Move Gamma Shuttle).", "Гамма шаттл", list("Да", "Нет")) != "Нет")
