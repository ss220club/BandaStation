...
/mob/living/basic/hostile/blackmesa/xen/headcrab
	...
	icon = 'modular_bandastation/mobs/icons/mob/headcrab.dmi'
	icon_state = "headcrab"
		...
		if(!is_zombie && isturf(src.loc))
			for(var/mob/living/carbon/human/H in oview(src, 1)) //Only for corpse right next to/on same tile
				if(H.stat == DEAD || (!H.check_death_method() && H.health <= HEALTH_THRESHOLD_DEAD))
					...
					break
		if(times_fired % 4 == 0)
			for(var/mob/living/basic/K in oview(src, 1)) //Only for corpse right next to/on same tile
...
/mob/living/basic/hostile/blackmesa/xen/headcrab/OpenFire(atom/A)
...
/mob/living/basic/hostile/blackmesa/xen/headcrab/proc/Zombify(mob/living/carbon/human/H)
...
/mob/living/basic/hostile/blackmesa/xen/headcrab/death()
	..()
	if(is_zombie)
		qdel(src)

/mob/living/basic/hostile/blackmesa/xen/headcrab/handle_automated_speech() // This way they have different screams when attacking, sometimes. Might be seen as sphagetthi code though.
	if(speak_chance)
		if(rand(0,200) < speak_chance)
			if(speak && speak.len)
				playsound(get_turf(src), pick(speak), 200, 1)

/mob/living/basic/hostile/blackmesa/xen/headcrab/Destroy()
	if(contents)
		for(var/mob/M in contents)
			M.loc = get_turf(src)
	return ..()

/mob/living/basic/hostile/blackmesa/xen/headcrab/update_icons()
	. = ..()
	if(is_zombie)
		overlays.Cut()
		overlays = human_overlays
		var/image/I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "headcrabpod")
		if(host_species == "Vox")
			I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "headcrabpod_vox")
		else if(host_species == "Gray")
			I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "headcrabpod_gray")
		overlays += I

/mob/living/basic/hostile/blackmesa/xen/headcrab/CanAttack(atom/the_target)
...
/mob/living/basic/hostile/blackmesa/xen/headcrab/fast/update_icons()
	. = ..()
	if(is_zombie)
		overlays.Cut()
		overlays = human_overlays
		var/image/I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "fast_headcrabpod")
		if(host_species == "Vox")
			I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "fast_headcrabpod_vox")
		else if(host_species == "Gray")
			I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "fast_headcrabpod_gray")
		overlays += I
...
/mob/living/basic/hostile/blackmesa/xen/headcrab/poison/update_icons()
	...
	if(is_zombie)
		overlays.Cut()
		overlays = human_overlays
		var/image/I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "poison_headcrabpod")
		if(host_species == "Vox")
			I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "poison_headcrabpod_vox")
		else if(host_species == "Gray")
			I = image('modular_bandastation/mobs/icons/mob/headcrab.dmi', icon_state = "poison_headcrabpod_gray")
		overlays += I


/mob/living/basic/hostile/blackmesa/xen/headcrab/poison/AttackingTarget()
	. = ..()
