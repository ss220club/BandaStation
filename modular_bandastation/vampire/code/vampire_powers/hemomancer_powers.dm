/datum/action/cooldown/spell/vampire_vamp_claws
	...
	desc = "Направьте магию крови, чтобы выковать смертоносные вампирские когти, высасывающие кровь и быстро наносящие удары. Нельзя использовать, если вы держите невыпадающий предмет."
...
/obj/item/vamp_claws/attack(mob/living/target, mob/living/user, params)
	...
	if(isalien(carbon_target) && !xenomorph_acid_boosted && carbon_target.ckey && carbon_target.stat != DEAD)
		...
		heal_boost = 2
		damtype = BURN
		hitsound = 'modular_bandastation/vampire/sound/weapons/sear.ogg'
