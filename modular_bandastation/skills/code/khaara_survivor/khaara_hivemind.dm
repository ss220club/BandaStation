...
/datum/khaara_hivemind
	...
	var/datum/team/khaara_hivemind/team
	/// Whether the hivemind is active
	var/active = TRUE

/datum/khaara_hivemind/New(datum/mind/infector)
	infector_ref = WEAKREF(infector)
	team = new(infector)
	team.hivemind_ref = WEAKREF(src)
	infector.add_antag_datum(/datum/antagonist/khaara_source, team)

	var/mob/living/infector_mob = infector.current
	if(infector_mob)
		var/datum/action/cooldown/khaara_hivemind_comm/comm = new(src)
		comm.Grant(infector_mob)
		var/datum/action/cooldown/khaara_hivemind_panel/panel = new(src)
		panel.Grant(infector_mob)

/datum/khaara_hivemind/Destroy()
	QDEL_NULL(team)
	for(var/datum/weakref/ref in thralls)
		var/datum/mind/thrall = ref.resolve()
		if(thrall)
			thrall.remove_antag_datum(/datum/antagonist/khaara_thrall)
	thralls.Cut()
	infector_ref = null
	return ..()

/// Returns the infector mind, or null
/datum/khaara_hivemind/proc/get_infector()
	return infector_ref?.resolve()

/// Returns the infector mob, or null
/datum/khaara_hivemind/proc/get_infector_mob()
	var/datum/mind/mind = get_infector()
	return mind?.current

/// Infects a target mob, making them a thrall
/datum/khaara_hivemind/proc/infect(mob/living/carbon/human/target, mob/living/infector)
	if(!target || !infector)
		return FALSE
	if(!target.mind)
	...
	if(HAS_MIND_TRAIT(target, TRAIT_UNCONVERTABLE))
		to_chat(infector, span_warning("Разум цели защищён от воздействия Кхара."))
		return FALSE
	...
	if(target.mind.has_antag_datum(/datum/antagonist/khaara_thrall))
		to_chat(infector, span_warning("Цель уже является частью улья Кхара."))
		return FALSE
	...
	target.mind.add_antag_datum(/datum/antagonist/khaara_thrall, team)
...
/datum/khaara_hivemind/proc/remove_thrall(datum/mind/thrall_mind, silent = FALSE)
	...
	if(thrall_mind)
		...
		thrall_mind.remove_antag_datum(/datum/antagonist/khaara_thrall)
		if(!silent)
			broadcast_message("[thrall_mind.name] исключён из улья Кхара.")

/// Kill a thrall remotely
/datum/khaara_hivemind/proc/kill_thrall(datum/mind/thrall_mind)
	...
	to_chat(target, span_userdanger("Ваша связь с ульем Кхара резко обрывается! Вирус в вашей крови начинает разрушать организм изнутри!"))
...
/datum/khaara_hivemind/proc/finish_kill(datum/mind/thrall_mind, datum/weakref/target_ref)
...
/datum/khaara_hivemind/proc/send_command(datum/mind/thrall_mind, message, mob/living/sender)
	if(!thrall_mind?.current)
		return
	to_chat(thrall_mind.current, span_boldnotice("Источник улья приказывает: \"[message]\""))
	if(sender != thrall_mind.current)
		log_directed_talk(sender, thrall_mind.current, message, LOG_SAY, "khaara hivemind command")

/// Broadcast a message to all thralls and the infector
/datum/khaara_hivemind/proc/broadcast_message(message, mob/living/sender)
	var/is_infector_message = is_infector(sender)
	var/formatted
	if(is_infector_message)
		formatted = "<i><font color='#ff5500' size='+3'>\[Улей Кхара\] [sender]: [message]</font></i>"
	else
		formatted = "<i><font color='#ff5500'>\[Улей Кхара\] [sender ? "[sender]: " : ""][message]</font></i>"
...
/datum/khaara_hivemind/proc/get_thralls_data()
	var/list/data = list()
...
/datum/khaara_hivemind/proc/on_thrall_mind_transferred(datum/mind/source, mob/old_body)
	SIGNAL_HANDLER
	if(old_body)
...
/datum/khaara_hivemind/proc/on_thrall_update(datum/source)
	SIGNAL_HANDLER
	update_ui()

/// Checks if the given mob is the infector
/datum/khaara_hivemind/proc/is_infector(mob/user)
	var/datum/mind/infector = get_infector()
	return infector?.current == user

/// Updates all open UIs
/datum/khaara_hivemind/proc/update_ui()
	var/datum/mind/infector = get_infector()
	if(!infector?.current)
		return
...
/datum/khaara_hivemind/proc/get_thrall_objective(datum/mind/thrall)
	var/datum/antagonist/khaara_thrall/antag = thrall?.has_antag_datum(/datum/antagonist/khaara_thrall)
	return antag?.thrall_objective || "Нет текущей цели"

/// Sets a new objective for a thrall
/datum/khaara_hivemind/proc/set_thrall_objective(datum/mind/thrall, objective_text)
	var/datum/antagonist/khaara_thrall/antag = thrall?.has_antag_datum(/datum/antagonist/khaara_thrall)
	if(!antag)
		return FALSE
...
/datum/khaara_hivemind/ui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "KhaaraHivemind", "Улей Кхара")
		ui.open()

/datum/khaara_hivemind/ui_state(mob/user)
	return GLOB.always_state

/datum/khaara_hivemind/ui_status(mob/user, datum/ui_state/state)
	if(!is_infector(user) && !check_rights_for(user.client, R_ADMIN))
		return UI_CLOSE
	return ..()

/datum/khaara_hivemind/ui_static_data(mob/user)
	var/list/data = list()
	data["infector_name"] = "[get_infector()?.name || "Неизвестно"]"
	return data

/datum/khaara_hivemind/ui_data(mob/user)
	var/list/data = list()
	var/datum/mind/infector = get_infector()

	data["infector_name"] = infector?.name || "Неизвестно"
...
/datum/khaara_hivemind/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return
...
/datum/khaara_hivemind/proc/find_thrall_mind(ref_string)
	for(var/datum/weakref/ref in thralls)
...
/datum/antagonist/khaara_source/create_team(datum/team/kh_team)
	team = kh_team

/datum/antagonist/khaara_source/get_team()
	return team

/datum/antagonist/khaara_source/Destroy()
	team = null
	return ..()

/datum/antagonist/khaara_source/apply_innate_effects(mob/living/mob_override)
	. = ..()
	var/mob/living/target = mob_override || owner.current
	add_team_hud(target, /datum/antagonist/khaara_thrall)

/datum/antagonist/khaara_source/on_gain()
	. = ..()
	ADD_TRAIT(owner.current, TRAIT_UNCONVERTABLE, REF(src))

/datum/antagonist/khaara_source/on_removal()
...
/datum/antagonist/khaara_thrall/create_team(datum/team/kh_team)
	team = kh_team

/datum/antagonist/khaara_thrall/get_team()
	return team

/datum/antagonist/khaara_thrall/Destroy()
	team = null
	return ..()

/datum/antagonist/khaara_thrall/apply_innate_effects(mob/living/mob_override)
	. = ..()
	var/mob/living/target = mob_override || owner.current
	add_team_hud(target, type)

/datum/antagonist/khaara_thrall/on_gain()
	. = ..()
	ADD_TRAIT(owner.current, TRAIT_FEARLESS, REF(src))

	var/datum/khaara_hivemind/hivemind = team?.hivemind_ref?.resolve()
	if(hivemind)
		var/datum/action/cooldown/khaara_hivemind_comm/comm = new(hivemind)
		comm.Grant(owner.current)
		hivemind.broadcast_message("[owner.name] принят в улей Кхара.")

/datum/antagonist/khaara_thrall/on_removal()
	. = ..()
