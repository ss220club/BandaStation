...
/datum/action/cooldown/spell/khaara_adrenaline_surge/cast(mob/living/cast_on)
	. = ..()

	if(cast_on.has_status_effect(/datum/status_effect/khaara_surge_active))
		to_chat(cast_on, span_warning("Вирусные остатки ещё не восстановились после предыдущего всплеска!"))
		return

	cast_on.apply_status_effect(/datum/status_effect/khaara_surge_active)
	cast_on.visible_message(
		...
		span_userdanger("Остатки вируса Кхара вскипают в вашей крови! Вы чувствуете чудовищный прилив сил!")
	)

/datum/status_effect/khaara_surge_active
	id = "khaara_surge_active"
	duration = 12 SECONDS
	tick_interval = 1 SECONDS
	status_type = STATUS_EFFECT_REPLACE
	alert_type = /atom/movable/screen/alert/status_effect/khaara_surge_active
	show_duration = TRUE
...
/datum/status_effect/khaara_surge_active/on_apply()
...
/datum/status_effect/khaara_surge_active/tick(seconds_between_ticks)
	var/mob/living/carbon/human/H = owner
	if(!H)
		return
...
/datum/status_effect/khaara_surge_active/on_remove()
	var/mob/living/carbon/human/H = owner
	if(!H)
		return

	H.remove_movespeed_modifier(/datum/movespeed_modifier/khaara_surge_speed, update = TRUE)
	...
	H.apply_status_effect(/datum/status_effect/khaara_surge_crash)

/datum/status_effect/khaara_surge_crash
	id = "khaara_surge_crash"
	duration = 10 SECONDS
	status_type = STATUS_EFFECT_REPLACE
	alert_type = /atom/movable/screen/alert/status_effect/khaara_surge_crash
	show_duration = TRUE
	processing_speed = STATUS_EFFECT_NORMAL_PROCESS

/datum/status_effect/khaara_surge_crash/on_apply()
	var/mob/living/carbon/human/H = owner
	if(!ishuman(H))
		return FALSE

	H.add_movespeed_modifier(/datum/movespeed_modifier/khaara_surge_crash, update = TRUE)
	to_chat(H, span_danger("Ваши мышцы дрожат от перенапряжения. Вы двигаетесь значительно медленнее."))

	return TRUE

/datum/status_effect/khaara_surge_crash/on_remove()
	var/mob/living/carbon/human/H = owner
	if(!H)
		return

	H.remove_movespeed_modifier(/datum/movespeed_modifier/khaara_surge_crash, update = TRUE)
	to_chat(H, span_notice("Вы восстанавливаете контроль над телом. Истощение отступает."))

/datum/movespeed_modifier/khaara_surge_speed
	multiplicative_slowdown = -0.12

/datum/movespeed_modifier/khaara_surge_crash
