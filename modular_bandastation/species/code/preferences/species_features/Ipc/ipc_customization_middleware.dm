...
/datum/preference_middleware/ipc_customization/get_constant_data(mob/user)
	...
	data["generations"] = list(
		list("key" = IPC_GEN_MODULAR,   "name" = "Поколение I"),
		list("key" = IPC_GEN_STANDARD,  "name" = "Поколение II"),
		list("key" = IPC_GEN_HUMANITY,  "name" = "Поколение III"),
		list("key" = IPC_GEN_CYBERDECK, "name" = "Поколение IV"),
	)

	// Список профессиональных модулей (для Поколения I)
	data["gen1_modules"] = list(
		list("key" = IPC_MODULE_MEDICAL,     "name" = "Медицинский"),
		list("key" = IPC_MODULE_ENGINEERING, "name" = "Инженерный"),
		list("key" = IPC_MODULE_SECURITY,    "name" = "Охранный"),
