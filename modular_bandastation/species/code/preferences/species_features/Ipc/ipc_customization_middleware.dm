...
/datum/preference_middleware/ipc_customization/get_constant_data(mob/user)
	...
	data["generations"] = list(
		...
		list(
			...
			"description" = "Продвинутая платформа КПБ. Может использовать обе руки независимо (минимальная задержка между руками). Повышенные скорость передвижения (+10%) и скорость действий (+15%). ЭМИ оглушает на 1-3 сек и наносит ожоги вместо паралича.",
		),
		list(
