/obj/item/mop/proc/attack_on_liquids_turf(obj/item/mop/the_mop, turf/target, mob/user, obj/effect/abstract/liquid_turf/liquids)
	if(!user.Adjacent(target))
		return FALSE
	var/speed_mult = 1
	var/datum/liquid_group/targeted_group = liquids?.liquid_group
	while(!QDELETED(targeted_group))
		...
		if(targeted_group?.reagents_per_turf)
			// Remove liquid from the tile we are actually mopping instead of draining the entire group uniformly.
			targeted_group.trans_to_seperate_group(
				the_mop.reagents,
				min(targeted_group.reagents_per_turf, 5, free_space),
				target.liquids
			)
