/obj/effect/abstract/liquid_turf
	name = "liquid"
	icon = 'modular_bandastation/liquids/icons/obj/effects/liquid.dmi'
	icon_state = "water-0"
...
/obj/effect/abstract/liquid_turf/proc/process_evaporation()
	...
	for(var/datum/reagent/reagent_type as anything in liquid_group.reagents.reagent_list)
		var/remove_amount = min(0.2 * liquid_group.evaporation_multiplier, reagent_type.volume, (liquid_group.reagents_per_turf / max(length(liquid_group.reagents.reagent_list), 1)))
		if(remove_amount <= 0)
			continue
		liquid_group.remove_specific(src, remove_amount, reagent_type, TRUE)
		any_change = TRUE
...
/obj/effect/abstract/liquid_turf/update_overlays()
	...
	if(number != 0)
		. += mutable_appearance('modular_bandastation/liquids/icons/obj/effects/liquid_overlays.dmi', "stage[number]_bottom", offset_spokesman = my_turf, plane = ABOVE_GAME_PLANE, layer = ABOVE_MOB_LAYER)
		. += mutable_appearance('modular_bandastation/liquids/icons/obj/effects/liquid_overlays.dmi', "stage[number]_top", offset_spokesman = my_turf, plane = GAME_PLANE, layer = WATER_LEVEL_LAYER)
	...
	if(liquid_group.group_overlay_state >= LIQUID_STATE_ANKLES)
		if(prob(30))
			var/sound_to_play = pick(list(
				'modular_bandastation/liquids/sound/effects/water_wade1.ogg',
				'modular_bandastation/liquids/sound/effects/water_wade2.ogg',
				'modular_bandastation/liquids/sound/effects/water_wade3.ogg',
				'modular_bandastation/liquids/sound/effects/water_wade4.ogg'
				))
		...
		if(isliving(AM))
			var/mob/living/stepped_living = AM
			liquid_group.expose_atom(stepped_living, 1, TOUCH)
	else if (isliving(AM))
		var/mob/living/L = AM
		if(liquid_group.slippery)
			if(L.move_intent == MOVE_INTENT_RUN && !(L.movement_type & FLYING) && L.body_position == STANDING_UP)
...
/obj/effect/abstract/liquid_turf/proc/mob_fall(datum/source, mob/M)
	...
	if(liquid_group.group_overlay_state >= LIQUID_STATE_ANKLES && T.has_gravity(T))
		playsound(T, 'modular_bandastation/liquids/sound/effects/splash.ogg', 50, 0)
		if(iscarbon(M))
			var/mob/living/carbon/C = M
...
/obj/effect/abstract/liquid_turf/proc/examine_turf(turf/source, mob/examiner, list/examine_list)
	...
	if(examiner.can_see_reagents())
		...
		if(length(liquid_group.reagents.reagent_list) == 1)
			...
			var/reagent_name = reagent_type.name
...
/obj/effect/temp_visual/liquid_splash
	icon = 'modular_bandastation/liquids/icons/obj/effects/splash.dmi'
	icon_state = "splash"
	layer = FLY_LAYER
	randomdir = FALSE
