...
/datum/liquid_group/proc/build_turf_reagent()
	...
	for(var/reagent_type in reagents.reagent_list)
		...
		if(amount > LIQUID_REAGENT_THRESHOLD_TURF_EXPOSURE)
			exposure = TRUE
		if(istype(pulled_reagent, /datum/reagent/lube))
			slippery = TRUE
...
/datum/liquid_group/proc/expose_atom(atom/target, modifier = 0, method)
	...
	if(HAS_TRAIT(target, LIQUID_PROTECTION))
		return
	turf_reagents.expose(target, method, modifier)
...
/datum/liquid_group/proc/spread_liquid(turf/new_turf, turf/source_turf)
	...
	if(QDELETED(new_turf.liquids) && source_turf.turf_height == new_turf.turf_height) // no space turfs, or oceans turfs, also don't attempt to spread onto a turf that already has liquids wastes processing time
		...
		new_turf.liquids.alpha = max(group_alpha, 120)
