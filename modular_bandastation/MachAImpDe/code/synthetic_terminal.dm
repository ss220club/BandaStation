...
/obj/machinery/computer/operating/synthetic/proc/get_patient_data(mob/living/carbon/human/patient)
	...
	if(istype(ipc_species))
		...
		if(ipc_species.cpu_temperature < ipc_species.cpu_temp_optimal_min)
			temp_status = IPC_TEMP_STATE_COLD
		else if(ipc_species.cpu_temperature > ipc_species.cpu_temp_optimal_max && ipc_species.cpu_temperature < IPC_TEMP_OVERHEAT_MID)
			temp_status = IPC_TEMP_STATE_WARM
		else if(ipc_species.cpu_temperature >= IPC_TEMP_OVERHEAT_MID && ipc_species.cpu_temperature < IPC_TEMP_OVERHEAT_HIGH)
			temp_status = IPC_TEMP_STATE_HOT
		else if(ipc_species.cpu_temperature >= IPC_TEMP_OVERHEAT_HIGH)
			temp_status = IPC_TEMP_STATE_CRITICAL
...
/obj/machinery/computer/operating/synthetic/proc/get_tool_display_name(tool_path)
	...
	// По умолчанию — читаем имя через initial() без создания объекта
	return initial(tool_path:name)
...
/obj/machinery/computer/operating/synthetic/proc/get_system_messages(mob/living/carbon/human/patient)
	...
	var/datum/species/ipc/ipc_species = patient.dna.species
	if(istype(ipc_species))
		if(ipc_species.cpu_temperature >= IPC_TEMP_CRITICAL_HIGH)
		...
		else if(ipc_species.cpu_temperature >= IPC_TEMP_OVERHEAT_MID)
			system_messages += list(list(
				"type" = "warning",
