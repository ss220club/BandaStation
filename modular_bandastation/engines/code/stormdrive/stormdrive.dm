...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor
	...
	icon = 'goonstation/icons/reactor.dmi'
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/attackby(obj/item/I, mob/living/carbon/user, params)
			...
			if(REACTOR_STATE_RUNNING)
				if(alert("[src] is not in maintenance mode! Manually inserting a control rod into an active nuclear reaction would probably be fatal.",name,"Continue","Reconsider") == "Continue" && Adjacent(user))
					if(control_rods.len >= MAX_CONTROL_RODS)
						...
						playsound(src, 'sound/items/tools/welder.ogg', 100, TRUE) //temp - find a better sound
						if(!do_after(user, 50, target = src))
							control_rod_installation = FALSE
							return
						to_chat(user, "<span class='notice'>You mount the [I.name] to the reactor control coupleing. </span>")
	...
	if(istype(I, /obj/item/stack/sheet/mineral/plastitanium))
		var/obj/item/stack/sheet/mineral/plastitanium/D = I
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/multitool_act(mob/living/user, obj/item/multitool/M)
	M.set_buffer(src)
	balloon_alert(user, "saved to multitool buffer")
	playsound(src, 'modular_bandastation/engines/sounds/flashlight_on.ogg', 100, TRUE)
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/blob_act(obj/structure/blob/B)
	if(state == REACTOR_STATE_RUNNING)
		if(heat < 400)
			reactor_stability -= B.max_integrity / 10
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/deactivate()
	...
	AR.ambient_buzz = 'modular_bandastation/engines/sounds/shipambience.ogg'
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/try_start()
	...
	var/fuel_check = air1.gases[GAS_PLASMA] * NORMAL_ROR + \
					...
					air1.gases[GAS_HYPER_NOBLIUM][MOLES] * REALLY_HINDER_ROR
	...
	if(fuel_check >= start_threshold && heat >= start_threshold) //Checking equivalent of 20 moles of fuel and is hot enough
		...
		set_light(5)
		var/startup_sound = pick('modular_bandastation/engines/sounds/startup.ogg', 'modular_bandastation/engines/sounds/startup2.ogg')
		playsound(loc, startup_sound, 100)
		send_alert("Fission reaction initiated. Reactor now on-line.", override=TRUE)
		var/area/AR = get_area(src)
		AR.ambient_buzz = 'modular_bandastation/engines/sounds/engineering.ogg'
		if(reaction_rate <= 0)
			reaction_rate = 5
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/slowprocess()
	...
	if(air1.total_moles() >= reaction_rate && fuel_check >= 12.5) //1:8 ratio
		...
		var/chamber_ror_total = reaction_chamber_gases.gases[GAS_PLASMA] * (NORMAL_ROR * plasma_modifier) + \
								...
								reaction_chamber_gases.gases[GAS_HYPER_NOBLIUM] * (REALLY_HINDER_ROR * nob_modifier)
		reaction_rate_modifier = chamber_ror_total / reaction_rate

		//checking for gas modifiers
		var/chamber_ipm_total = reaction_rate + reaction_chamber_gases.gases[GAS_TRITIUM] * (HIGH_IPM * tritium_modifier) + \
												reaction_chamber_gases.gases[GAS_O2] * (HIGH_IPM * o2_modifier) + \
												reaction_chamber_gases.gases[GAS_PLUOXIUM] * (HIGH_IPM * pluoxium_modifier) + \
												reaction_chamber_gases.gases[GAS_NITRIUM] * (VERY_HIGH_IPM * stim_modifier) - \
												reaction_chamber_gases.gases[GAS_PLASMA] * (MEDIOCRE_IPM * plasma_modifier) - \
												reaction_chamber_gases.gases[GAS_CO2] * (LOW_IPM * co2_modifier) - \
												reaction_chamber_gases.gases[GAS_HYPER_NOBLIUM] * (LOW_IPM * nob_modifier)
		input_power_modifier = chamber_ipm_total / reaction_rate

		var/chamber_cooling_total = reaction_rate + reaction_chamber_gases.gases[GAS_HYPER_NOBLIUM] * (VERY_HIGH_COOLING * nob_modifier) + \
													reaction_chamber_gases.gases[GAS_HEALIUM] * (HIGH_COOLING * no2_modifier) + \
													reaction_chamber_gases.gases[GAS_N2] * (HIGH_COOLING * n2_modifier) + \
													reaction_chamber_gases.gases[GAS_CO2] * (HIGH_COOLING * co2_modifier) - \
													reaction_chamber_gases.gases[GAS_TRITIUM] * (LOW_COOLING * tritium_modifier) - \
													reaction_chamber_gases.gases[GAS_NUCLEIUM] * (LOW_COOLING * nucleium_modifier) - \
													reaction_chamber_gases.gases[GAS_NITRIUM] * (LOW_COOLING * stim_modifier)
		cooling_power_modifier = chamber_cooling_total / reaction_rate

		var/chamber_radiation_total = reaction_rate + reaction_chamber_gases.gases[GAS_TRITIUM] * (VERY_HIGH_RADIATION * tritium_modifier) + \
													reaction_chamber_gases.gases[GAS_PLASMA] * (HIGH_RADIATION * plasma_modifier) + \
													reaction_chamber_gases.gases[GAS_NUCLEIUM] * (HIGH_RADIATION * nucleium_modifier) + \
													reaction_chamber_gases.gases[GAS_BZ] * (HIGH_RADIATION * bz_modifier) - \
													reaction_chamber_gases.gases[GAS_NITRIUM] * (LOW_RADIATION * stim_modifier) - \
													reaction_chamber_gases.gases[GAS_HEALIUM] * (LOW_RADIATION * no2_modifier)
		radiation_modifier = chamber_radiation_total / reaction_rate

		var/chamber_reinforcement_total = reaction_rate + reaction_chamber_gases.gases[GAS_PLUOXIUM] * (VERY_HIGH_REINFORCEMENT * pluoxium_modifier) + \
														reaction_chamber_gases.gases[GAS_TRITIUM] * (HIGH_REINFORCEMENT * tritium_modifier) + \
														reaction_chamber_gases.gases[GAS_N2O] * (HIGH_REINFORCEMENT * no2_modifier) - \
														reaction_chamber_gases.gases[GAS_NUCLEIUM] * (LOW_REINFORCEMENT * nucleium_modifier) - \
														reaction_chamber_gases.gases[GAS_NITRIUM] * (LOW_REINFORCEMENT * stim_modifier) - \
														reaction_chamber_gases.gases[GAS_BZ] * (LOW_REINFORCEMENT * bz_modifier)
		reactor_temperature_modifier = chamber_reinforcement_total / reaction_rate

		var/chamber_degradation_total = reaction_rate + reaction_chamber_gases.gases[GAS_N2O] * (HIGH_DEG_PROTECTION * no2_modifier) + \
														reaction_chamber_gases.gases[GAS_HYPER_NOBLIUM] * (VERY_HIGH_DEG_PROTECTION * nob_modifier) + \
														reaction_chamber_gases.gases[GAS_PLUOXIUM] * (HIGH_DEG_PROTECTION * pluoxium_modifier)
		control_rod_degradation_modifier = chamber_degradation_total / reaction_rate

		nucleium_power_reduction = reaction_chamber_gases.gases[/datum/gas/nucleium][MOLES] *= 1000 //Suffering get_moles(GAS_NUCLEIUM) * 1000 //nucleium

		heat_gain = initial(heat_gain) + reaction_rate
		reaction_chamber_gases.remove()
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/handle_ftl_fuel_production()
	if(heat > initial(reactor_temperature_hot))
		...
		if(output_starting_pressure >= max_output_pressure) //if pressured capped, nucleium backs up into the drive
			...
			air2.assert_gases(GAS_NUCLEIUM, (reaction_rate / 10) * input_power_modifier)
		...
		switch(reactor_stability)
				...
				if(C.get_bodypart(BODY_ZONE_HEAD))
					...
					if(affecting && affecting.receive_damage(5)) //minor brute damage
						if(!istype(C.head, /obj/item/clothing/head/helmet))
							C.adjust_organ_loss(ORGAN_SLOT_BRAIN, 5, 60)
			...
			if(25 to 50) //Flesh for the Fleshdrive
				if(C.get_bodypart(BODY_ZONE_L_LEG))
					...
					var/datum/gas_mixture/air1 = airs[1]
					air1.assert_gases(GAS_PLASMA, 25)
					return
			...
			if(-INFINITY to 25) //Souls for the Souldrive
				...
				var/datum/gas_mixture/air1 = airs[1]
				air1.assert_gases(GAS_PLASMA, 100)
				return
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/check_meltdown_warning()
	...
	if(heat >= reactor_temperature_critical)
		...
		sound_to_playing_players('modular_bandastation/engines/sounds/core_overheating.ogg')
		return TRUE
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/update_icon_state() //include overlays for radiation output levels and power output levels (ALSO 1k+ levels)
	if(state == REACTOR_STATE_MELTDOWN)
		icon_state = "broken"
		return ..()
	if(state == REACTOR_STATE_REPAIR)
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/start_meltdown()
	...
	sound_to_playing_players('modular_bandastation/engines/sounds/meltdown.ogg')
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/meltdown()
	if(heat >= reactor_temperature_meltdown)
		...
		sound_to_playing_players('modular_bandastation/engines/sounds/explode.ogg')
		cut_overlays()
		flick("meltdown", src)
		do_meltdown_effects()
		sleep(10)
...
/obj/machinery/computer/ship/reactor_control_computer
	...
	icon = 'modular_bandastation/engines/icons/reactor/computer.dmi'
...
/obj/machinery/computer/ship/reactor_control_computer/multitool_act(mob/living/user, obj/item/multitool/M)
	. = NONE

	if(istype(M.buffer, /obj/machinery/atmospherics/components/binary/stormdrive_reactor))
		reactor = M.buffer
		to_chat(user, span_notice("Вы привязываете [declent_ru(NOMINATIVE)] через [M.buffer.declent_ru(NOMINATIVE)]."))
		M.set_buffer(null)
...
/obj/machinery/computer/ship/reactor_control_computer/attack_hand(mob/user)
	if(!allowed(user))
		var/sound = pick('modular_bandastation/engines/sounds/error.ogg','modular_bandastation/engines/sounds/error2.ogg','modular_bandastation/engines/sounds/error3.ogg')
...
/obj/machinery/computer/ship/reactor_control_computer/Initialize(mapload)
	...
	if(reactor_id) //If mappers set an ID)
		for(var/obj/machinery/atmospherics/components/binary/stormdrive_reactor/sdr in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/atmospherics/components/binary/stormdrive_reactor))
...
/obj/machinery/computer/ship/reactor_control_computer/ui_data(mob/user)
	...
	data["o2"] = air1 ? air1.gases[GAS_O2] : 0
	data["n2"] = air1 ? air1.gases[GAS_N2] : 0
	data["co2"] = air1 ? air1.gases[GAS_CO2] : 0
	data["plasma"] = air1 ? air1.gases[GAS_PLASMA] : 0
	data["water_vapour"] = air1 ? air1.gases[GAS_WATER_VAPOR] : 0
	data["nob"] = air1 ? air1.gases[GAS_HYPER_NOBLIUM] : 0
	data["n2o"] = air1 ? air1.gases[GAS_N2O] : 0
	data["no2"] = air1 ? air1.gases[GAS_HEALIUM] : 0
	data["tritium"] = air1 ? air1.gases[GAS_TRITIUM] : 0
...
/datum/weather/nuclear_fallout
	...
	weather_sound = 'modular_bandastation/engines/sounds/falloutwind.ogg'
	end_duration = 100
	area_type = /area
...
/datum/weather/nuclear_fallout/weather_act_mob(mob/living/L)
	if(!SSradiation.can_irradiate_basic(L) || SSradiation.wearing_rad_protected_clothing(L))
		return
...
/obj/effect/anomaly/stormdrive/attackby(obj/item/I, mob/user, params) //Not going to make this easy
	if(I.tool_behaviour == TOOL_ANALYZER)
		...
		if(prob(50))
			var/freq_1 = format_frequency(anomaly_core.frequency - freq_shift)
			var/freq_2 = format_frequency(anomaly_core.frequency + freq_shift)
			to_chat(user, "<span class='notice'>Analyzing... ERROR: Unable to #%##freq0x000[mem_address] [freq_1] - [freq_2].</span>")
		else
			var/code_1 = anomaly_core.code - code_shift
			var/code_2 = anomaly_core.code + code_shift
...
/obj/effect/anomaly/stormdrive/surge/proc/mobShock(mob/living/M)
	if(canshock && istype(M))
		...
		if(iscarbon(M))
			...
			M.adjust_fire_loss(shockdamage)
...
/obj/item/stormdrive_core
	...
	icon = 'modular_bandastation/engines/icons/rods/control_rod.dmi'
...
/datum/computer_file/program/stormdrive_monitor/process_tick()
	...
	program_open_overlay = "smmon_[stage]"
	if(istype(computer))
...
/datum/computer_file/program/stormdrive_monitor/on_start(mob/living/user)
	...
	if(!reactor)
		for(var/obj/machinery/atmospherics/components/binary/stormdrive_reactor/R in SSmachines.get_machines_by_type_and_subtypes(/obj/machinery/atmospherics/components/binary/stormdrive_reactor))
			reactor = R
			break
	active = TRUE
