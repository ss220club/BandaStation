...
/*
// MARK: God bless Chat LGBT
/obj/effect/music_emitter
	name = "music emitter"
	anchored = TRUE
	...
	var/volume = 80
	var/channel = 47
	var/check_period = 2 SECONDS
	var/tick_period  = 0.5 SECONDS
	var/fade_time    = 1.5 SECONDS

	var/list/current_vol = list()
	var/list/target_vol = list()
	var/list/attached = list()
	var/playing = FALSE

/obj/effect/music_emitter/Initialize(mapload)
	. = ..()
	start()
	addtimer(CALLBACK(src, PROC_REF(reconcile_listeners)), check_period, TIMER_LOOP | TIMER_STOPPABLE)
	addtimer(CALLBACK(src, PROC_REF(process_fades)), tick_period, TIMER_LOOP | TIMER_STOPPABLE)

/obj/effect/music_emitter/Destroy()
	stop()
	return ..()

/obj/effect/music_emitter/proc/start()
	if(playing || !sound_file) return
	playing = TRUE
	reconcile_listeners()

/obj/effect/music_emitter/proc/stop()
	if(!playing) return
	for(var/mob/M in attached)
		if(M?.client)
			SEND_SOUND(M, sound(null, channel = channel))
	current_vol.Cut(); target_vol.Cut(); attached.Cut()
	playing = FALSE

/obj/effect/music_emitter/proc/reconcile_listeners()
	if(!playing) return
	var/list/in_radius = list()
	for(var/mob/living/M in hearers(radius, src))
		if(!M?.client) continue
		in_radius[M] = TRUE
		if(!(M in attached))
			send_sound_start(M, 0)
			attached[M] = TRUE
			current_vol[M] = 0
		target_vol[M] = volume
	for(var/mob/L in current_vol.Copy())
		if(!(L in in_radius) || !L?.client)
			target_vol[L] = 0

/obj/effect/music_emitter/proc/process_fades()
	if(!playing) return
	var/step = (tick_period / max(0.1 SECONDS, fade_time))
	for(var/mob/M in current_vol.Copy())
		if(!M?.client) { drop_listener(M); continue }
		var/cv = current_vol[M]
		var/tv = isnull(target_vol[M]) ? 0 : target_vol[M]
		if(cv == tv) continue
		var/delta = (tv - cv) * step
		if(abs(delta) < 1) delta = (tv > cv) ? 1 : -1
		var/nv = clamp(round(cv + delta), 0, 100)
		if(nv != cv)
			current_vol[M] = nv
			send_sound_update(M, nv)

/obj/effect/music_emitter/proc/drop_listener(mob/M)
	if(!M) return
	SEND_SOUND(M, sound(null, channel = channel))
	current_vol -= M
	target_vol  -= M
...
/obj/effect/music_emitter/proc/send_sound_start(mob/M, vol)
	if(!M?.client) return
...
/obj/effect/music_emitter/proc/send_sound_update(mob/M, vol)
	if(!M?.client) return
	var/sound/S = sound(sound_file)
	S.channel = channel
	S.repeat = TRUE
