...
/obj/structure/deployable_barricade/update_overlays()
	...
	if(is_wired)
		if(!closed)
			. += image('modular_bandastation/voyaker_events/icons/barricade.dmi', icon_state = "[barricade_type]_wire")
		else
...
/obj/structure/deployable_barricade/metal/update_overlays()
	...
	switch(barricade_upgrade_type)
		if(BARRICADE_TYPE_BOMB)
			. += image('modular_bandastation/voyaker_events/icons/barricade.dmi', icon_state = "+explosive_upgrade_[damage_state]")
		if(BARRICADE_TYPE_MELEE)
			. += image('modular_bandastation/voyaker_events/icons/barricade.dmi', icon_state = "+brute_upgrade_[damage_state]")
		if(BARRICADE_TYPE_ACID)
			. += image('modular_bandastation/voyaker_events/icons/barricade.dmi', icon_state = "+burn_upgrade_[damage_state]")
		...
		for(var/direction in GLOB.cardinals)
			for(var/obj/structure/deployable_barricade/metal/plasteel/cade in get_step(src, direction))
				if(((dir & (NORTH|SOUTH) && get_dir(src, cade) & (EAST|WEST)) || (dir & (EAST|WEST) && get_dir(src, cade) & (NORTH|SOUTH))) && dir == cade.dir && cade.linked && cade.closed == closed)
					. += image('modular_bandastation/voyaker_events/icons/barricade.dmi', icon_state = "[barricade_type]_[closed ? "closed" : "open"]_connection_[get_dir(src, cade)]")
