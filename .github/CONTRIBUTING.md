...
Переводы имён предметов хранятся в `modular_bandastation/translations/code/translation_data/ru_names/` - по одному `.toml` файлу на каждое имя. `ru_names.toml` генерируется автоматически при сборке, не редактируйте его напрямую.
...
Поле `nominative` обязательное - остальные падежи используют его как fallback если не заполнены.
